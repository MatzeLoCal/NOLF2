...
void CProjectile::Detonate(HOBJECT hObj)
{
...
#if defined(LT_MACOS)
	// LT_TRACE_WEAPON=1 — pairs with the [wpn] fire trace: this is the other
	// end of a shot, so "N fires / N detonations" is the proof that projectiles
	// are actually landing and not just being spawned.
	if (getenv("LT_TRACE_WEAPON"))
	{
		LTVector vDbg;
		g_pLTServer->GetObjectPos(m_hObject, &vDbg);
		fprintf(stderr, "[prj] Detonate hObj=%p at (%.0f %.0f %.0f)%s\n",
		        (void*)hObj, vDbg.x, vDbg.y, vDbg.z, hObj ? "" : "  (lifetime expired)");
	}
#endif
	...
	if (hObj)
	{
		HOBJECT hDamager = m_hFiredFrom ? (HOBJECT)m_hFiredFrom : m_hObject;
