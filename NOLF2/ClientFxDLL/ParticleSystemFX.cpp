...
bool CParticleSystemFX::Init( ILTClient *pLTClient, FX_BASEDATA *pData, const CBaseFXProps *pProps)
{
	...
	// §54: is the AUTHORED BlendMode reaching the object? The welder's flame
	// authors Additive; every system was drawing with alpha blending.
	if (getenv("LT_TRACE_FX"))
		fprintf(stderr, "[fx]   ParticleSystem obj: props.blend=0x%x dataFlags2=0x%x -> ocs.m_Flags2=0x%x\n",
		        (unsigned)GetProps()->m_dwBlendMode,
		        (unsigned)pData->m_dwObjectFlags2, (unsigned)ocs.m_Flags2);
