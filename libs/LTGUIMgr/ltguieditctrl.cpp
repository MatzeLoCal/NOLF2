...
// Handles a key press
// LT_TRACE_KEYS=1 — the text-input path (§52). Cached: this is on the keypress
// path, and a getenv per keystroke is silly.
static bool gui_TraceKeys()
{
	static int s_n = -1;
	if (s_n < 0) s_n = getenv("LT_TRACE_KEYS") ? 1 : 0;
	return s_n != 0;
}
...
{
	if (gui_TraceKeys())
		fprintf(stderr, "[keys] EditCtrl::HandleKeyDown key=%d caret=%d len=%d text='%s'\n",
		        key, (int)m_nCaretPos,
		        m_pText ? (int)m_pText->GetLength() : -1,
		        (m_pText && m_pText->GetText()) ? m_pText->GetText() : "<null>");
	...
	case VK_BACK:
		if (m_nCaretPos > 0)
		{
			...
			RemoveCharacter();
			if (gui_TraceKeys())
				fprintf(stderr, "[keys]   VK_BACK -> caret=%d len=%d text='%s'\n",
				        (int)m_nCaretPos, m_pText ? (int)m_pText->GetLength() : -1,
				        (m_pText && m_pText->GetText()) ? m_pText->GetText() : "<null>");
		}
		break;
	case VK_DELETE:
