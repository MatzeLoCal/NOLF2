...
#if defined(LT_MACOS)
			// LT_TRACE_LIGHTFX=1 -- the CLIENT end of the dynamic-light chain
			// (server ClientLightFX -> SFX msg -> CLightFX -> OT_LIGHT object).
			// Pairs with the [lfx-srv] trace in ObjectDLL/ClientLightFX.cpp, so
			// one run says whether the message never arrives or arrives and the
			// object is not created.
			if (getenv("LT_TRACE_LIGHTFX"))
			{
				fprintf(stderr, "[lfx-cli] SFX_LIGHT_ID msg: color=(%.0f %.0f %.0f) flags=0x%X "
				                "radius=%.0f..%.0f intensity=%.2f..%.2f\n",
				        light.vColor.x, light.vColor.y, light.vColor.z, light.dwLightFlags,
				        light.fRadiusMin, light.fRadiusMax,
				        light.fIntensityMin, light.fIntensityMax);
			}
#endif
