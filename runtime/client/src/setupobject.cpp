    ...
    *ppSprite = LTNULL;
    ...
    if (pFileRef)
    {
        ...
        else
        {
            ...
            if (!pSprite)
            {
                // ★ ALWAYS-ON. spr_Create fails silently otherwise (SetupError
                // and RETURN_ERROR_PARAM both go nowhere in a retail build,
                // §5), and until the TieOff fix in spr_Create this path did not
                // just lose the sprite — it CRASHED in spr_Destroy. A sprite
                // that will not parse is worth a line either way: it means a
                // lens flare, muzzle flash or glow is simply missing.
                fprintf(stderr, "[sprite] LOAD FAILED (spr_Create): '%s'\n",
                        pFilenameString ? pFilenameString : "<null>");
...
{
	// No render-style provider in the build (the null/GL bring-up renderer);
	// leave the style NULL — only the renderer's model draw path consumes it.
	if (!renderstyles)
		return LT_OK;
...
// ★ The interned `models\\default.ltb` Model, recorded the first time the engine
// substitutes it for a model that is missing or unspecified. The renderer uses
// this for POINTER-IDENTITY comparison so it can refuse to draw the placeholder
// in the player-view pass, where a 1-unit cube at the camera whites out the
// whole screen (the holster bug). NULL until a substitution actually happens.
Model *g_pPlaceholderModel = LTNULL;
								...
								bool				 bLocalFromServer)
{
	...
	if(!bLocalFromServer && IsFileValid(pSetup->m_Filename[0]))
	{
		...
		{
			// ★ A MODEL THAT FAILS TO LOAD IS SILENTLY REPLACED BY A 1-UNIT CUBE.
			// DEBUG_MODEL_REZ is compiled out in a retail build, so this looked
			// like "the effect renders wrong" rather than "half the geometry is
			// missing" -- that is how the C01S01 waterfall lost WATERFALL2.LTB
			// (its broad sheet) and read as a thin ribbon. Always report it.
			fprintf(stderr, "[model] LOAD FAILED: '%s' (id %d) -> substituting models\\default.ltb\n",
			        pSetup->m_Filename[0].m_pFilename ? pSetup->m_Filename[0].m_pFilename : "<null>",
			        (int)pSetup->m_Filename[0].m_FileID);
	...
	else
	{
		...
		if( !pModel )
		{
			// ★★ "NO MODEL WANTED" IS NOT "MODEL FAILED TO LOAD".
			//
			// The placeholder cube exists to make a MISSING ASSET obvious — that
			// is how FX\WATERFALL2.LTB was finally caught (§36). But this branch
			// is reached when NO FILENAME WAS SPECIFIED AT ALL, which is a
			// deliberate "this object has no model", and substituting a visible
			// 1-unit box for it is actively wrong.
			//
			// ⚠️ THE HOLSTER WHITE-OUT. WEAPONS.TXT [Weapon30] is literally named
			// "Holster" and authors PVModel = "" (an empty string, not a missing
			// line). Holstering switches to it; CClientWeapon::CreateWeaponModel
			// (ClientWeapon.cpp:2699) has no empty-name guard and creates the
			// object anyway; it landed here and came back bound to
			// models\default.ltb with FLAG_VISIBLE|FLAG_REALLYCLOSE. In the
			// player-view pass — near plane 0.1, model ~0.2 units from the camera
			// — that box straddles the near plane, clips open, and fills the
			// screen with its untextured interior. Measured with LT_TRACE_PV=1:
			//     [pv] models/default.ltb pos(-0.22 -0.33 0.32) flags=0x41 pieces=1
			//
			// ⚠️⚠️ FAILING THE CREATION HERE IS NOT AN OPTION — MEASURED, CRASHES.
			// The obvious fix (return an error so the object is never made) looks
			// right: so_ExtraInit's error propagates to CClientMgr
			// (clientmgr.cpp:2040), the object is destroyed, CreateObject returns
			// 0, CreateWeaponModel returns false and Activate() calls Term(). But
			// CClientWeapon::Select() does NOT tolerate a failed Activate: it
			// carries on into SetDisable() -> HasAmmo(), which dereferences the
			// m_pAmmo that Term() just cleared. Segfault during the c01 cinematic,
			// every time. The object MUST be created successfully.
			//
			...
			if( dResult == LT_OK )
			{
				pModelInstance->BindModelDB(pModel);
				pModelInstance->InitAnimTrackers();
				// Record WHICH interned Model is the placeholder so the renderer
				// can recognise it by POINTER IDENTITY rather than by comparing
				// filename strings (the handoff's "beware substring greps").
				g_pPlaceholderModel = pModel;
			}
    ...
    for (i=0; i < MAX_MODEL_RENDERSTYLES; i++)
	{
		...
		{
			if (pModelInstance->m_pRenderStyles[i] && renderstyles)
			{
