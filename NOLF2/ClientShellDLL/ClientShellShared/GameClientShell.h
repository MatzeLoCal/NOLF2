...
public:
    ...
    void        ShowLevelName(bool bShow=true)	{ m_bShowLevelName = bShow; }
...
// should be added here (to keep them together)...
    ...
    bool      m_bShowLevelName;       // Display current level name (whereismymonkey).
	...
	// eDSTopRight is a macOS port addition — a large, fixed-size slot in the
	// UPPER right, away from the retail bottom-row debug text.
	enum DSSL { eDSBottomLeft, eDSBottomRight, eDSTopRight };

	// Point size for eDSTopRight, before the resolution ratio is applied.
	enum { kTopRightDebugPointSize = 24 };
	...
	CUIPolyString* 	m_pTopRightDebugString[kMaxDebugStrings];
