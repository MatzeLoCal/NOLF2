...
class CCheatMgr
{
	public:
        ...
        // Check() takes a non-const CParsedMsg&; MSVC bound the temporary
        // directly, Clang needs a named lvalue.
        LTBOOL  Check( const char* pMsg ) { if (!(pMsg && pMsg[0])) return LTFALSE; CParsedMsg cMsg(1, (const char * const *)&pMsg); return Check(cMsg); }
	...
	private:
        ...
        void    WhereAmI(LTBOOL bMode);   // macOS port addition: show level name
		void	SetKFA();
		void	SetFullWeapons();
