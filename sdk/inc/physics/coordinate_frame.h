...
public:
	...
	*/
	const LTVector3f TransformPointToParent( const LTVector3f& p ) const
	{
		...
		// this-> required: dependent-base (T) members are not visible to
		// unqualified lookup under two-phase name lookup (Clang/GCC).
		return this->TransformVectorToParent( p ) + this->m_O;
	}
	...
	*/
	void Rotate( const float a, const LTVector3f& u, const LTVector3f& p0 )
	{
		...
		// M * vector yields a vector (LTMatrix::operator*(LTVector) -> LTVector).
		// Original declared this as LTMatrix3f, which only compiled via MSVC's
		// implicit vector->matrix conversion and then mis-cast back to a vector.
