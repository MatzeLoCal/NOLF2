...
// WAVEFORMAT / WAVEFORMATEX (Win32 mmreg types). On Windows these arrive via
// windows.h; on macOS the compat shim supplies the same structs.
#include <windows.h>
...
// ★★★ `int`, NOT `long` — THIS WAS §29 (see PHASE2_HANDOFF §74).
//
// `long` is 4 bytes on Win32 and **8 on LP64**, so on arm64 this "32-bit" type
// was 64-bit — while its own unsigned counterpart on the next line is
// `unsigned int` and stayed 4. The two halves of the same pair had DIFFERENT
// WIDTHS, which is the tell that one of them is wrong.
//
// What it cost: every `*(S32*)pVal = x` in a sound driver wrote EIGHT bytes
// into whatever the engine had passed. `CSoundMgr::Init` passes `&nSamples`, a
// genuine `uint32` local (soundmgr.cpp:405), and the four bytes of overspill
// landed on the adjacent spilled `soundInit` reference, zeroing its LOW half:
// `0x000000016FDFB328` -> `0x0000000100000000`. Init then wrote through it at
// line 508 and died at `0x100000120`. That is the crash that kept 3D sound
// gated off, and with 3D sound off every positional sound crammed into the
// 28-slot 2D pool and retriggered ~20x/second -- the crackle the user reported.
//
// ⚠️ Caught with a watchpoint, not by reading code: the low-half-zeroed fault
// address is the port's recurring LP64 signature (family #2), and the writer
// was named in one run. `LTSOUNDUSERDATA` is already `intptr_t`, so nothing
// stores a pointer in this type and narrowing it is safe.
...
// ⚠️ USER-DATA VALUES ARE POINTER-SIZED (LT_MACOS / LP64).
// The engine smuggles POINTERS through these slots -- CSample*, CSoundBuffer*
// and CSoundInstance* (soundmgr.cpp SAMPLE_LISTITEM / SAMPLE_BUFFER /
// SAMPLE_INSTANCE) -- and the original signature was sint32, so on LP64 every
// one of them was truncated to its low 32 bits and cast straight back to a
// pointer. That crashed in CSoundMgr::ReleaseSWSample the moment a real sound
// driver existed. Same idiom as the MID_TRIGGER `Writeuint32((uint32)p)` bug
// (PHASE2_HANDOFF §5); widened here in lockstep with all 8 call sites.
// NOTE: sys/s_dx8 and sys/s_nul still carry the old sint32 signatures. Neither
// is built on macOS; reviving either means widening it to match.
#include <stdint.h>
typedef intptr_t LTSOUNDUSERDATA;
...
public:
	...
	virtual void		Set3DUserData( LH3DPOBJECT hObj, uint32 uiIndex, LTSOUNDUSERDATA siValue ) = 0;
	...
	virtual LTSOUNDUSERDATA		Get3DUserData( LH3DPOBJECT hObj, uint32 uiIndex) = 0;
	...
	virtual void		SetSampleUserData( LHSAMPLE hS, uint32 uiIndex, LTSOUNDUSERDATA siValue ) = 0;
	...
	virtual LTSOUNDUSERDATA		GetSampleUserData( LHSAMPLE hS, uint32 uiIndex ) = 0;
	...
	virtual void		SetStreamUserData( LHSTREAM hS, uint32 uiIndex, LTSOUNDUSERDATA siValue) = 0;
	virtual LTSOUNDUSERDATA		GetStreamUserData( LHSTREAM hS, uint32 uiIndex) = 0;
