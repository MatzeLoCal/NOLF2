...
// LTVector2 (below) calls LTMIN/LTMAX, whose template definitions live later in
// ltbasedefs.h -- after this header is pulled in via the ltbasetypes.h chain.
// Forward-declare them so clang's two-phase name lookup resolves them at the
// point of the template definition (MSVC did not require this).
template<class T, class TB> T LTMIN(T a, TB b);
template<class T, class TB> T LTMAX(T a, TB b);
