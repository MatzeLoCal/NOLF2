...
void sm_TellClientAboutGlobalLight(Client *pClient)
{
	...
	if (getenv("LT_TRACE_SUN"))
		fprintf(stderr, "[sun] InnerColor=(%.0f %.0f %.0f) BrightScale=%.3f ObjectBrightScale=%.3f"
		                " -> sent=(%.1f %.1f %.1f) convertToAmbient=%.2f\n",
		        pSun->m_InnerColor.x, pSun->m_InnerColor.y, pSun->m_InnerColor.z,
		        pSun->m_BrightScale, pSun->m_ObjectBrightScale,
		        pSun->m_InnerColor.x * fBrightness, pSun->m_InnerColor.y * fBrightness,
		        pSun->m_InnerColor.z * fBrightness, pSun->m_fConvertToAmbient);
