...
bool CSnowFX::UpdateAirspaces( void )
{
	...
	if( !m_bEnabled )
	{
		// ⚠️ Deactivate() erases the airspace from activeAirspaces -- the set
		// being iterated. Advance the iterator BEFORE deactivating, or ++ runs
		// on a freed tree node. MSVC's std::set happened to survive this;
		// libc++ crashes deterministically (the Siberia outpost segfault).
		std::set<CSnowFXAirspace*>::iterator it = activeAirspaces.begin();
		while( it != activeAirspaces.end() )
		{
			CSnowFXAirspace* pAirspace = *it;
			++it;
			pAirspace->Deactivate();
		}
	...
	if( cameraDistSq > m_fMaxDrawDistSq )
	{
		...
		// (same iterator-invalidation hazard as above: Deactivate() erases from
		// this set, so advance first)
		std::set<CSnowFXAirspace*>::iterator it = activeAirspaces.begin();
		while( it != activeAirspaces.end() )
		{
			CSnowFXAirspace* pAirspace = *it;
