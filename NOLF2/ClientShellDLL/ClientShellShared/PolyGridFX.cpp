...
// LT_TRACE_PGFX=1: report WHICH early return stops the water animating.
// Update() has seven of them and every one is silent; "the surface never moves"
// is the only symptom all seven share. Reports on CHANGE (plus a slow
// heartbeat), never once — a one-shot cannot tell "blocked" from "blocked at a
// different gate now".
static void pgfx_Gate(const char* pszWhere)
{
	static int s_nOn = -1;
	if (s_nOn < 0) s_nOn = getenv("LT_TRACE_PGFX") ? 1 : 0;
	if (!s_nOn) return;

	static const char* s_pszLast = "";
	static uint32 s_nCalls = 0;
	++s_nCalls;
	if (s_pszLast != pszWhere || (s_nCalls % 300) == 0)
	{
		fprintf(stderr, "[pgfx] Update -> %s (call %u)\n", pszWhere, s_nCalls);
		s_pszLast = pszWhere;
	}
}
...
LTBOOL CPolyGridFX::Update()
{
	...
	{
		pgfx_Gate("STOP: no object / no server object");
		return LTFALSE;
	}
	...
	else  // We're hidden, no need to update...
	{
		...
		pgfx_Gate("STOP: server object not FLAG_VISIBLE");
        return LTTRUE;
	}
	...
	{
		// The renderer must set FLAG_INTERNAL1 for CClientMgr::UpdatePolyGrids
		// to promote it to FLAG_WASDRAWN (clientmgr.cpp:2209).
		pgfx_Gate("STOP: !FLAG_WASDRAWN (renderer never set FLAG_INTERNAL1?)");
        return LTTRUE;
	}
	...
	{
		CGameSettings* pSettings = g_pInterfaceMgr->GetSettings();
        if (!pSettings) { pgfx_Gate("STOP: no settings"); return LTTRUE; }
		...
		if (!bOn || nVal == RS_LOW)
		{
			pgfx_Gate(bOn ? "STOP: SpecialFXSetting == RS_LOW"
			              : "STOP: settings PolyGrids() is OFF");
            return LTTRUE;
		}
	}
	...
	// ★ The update RAN — but running is not the same as changing anything.
	// UpdatePlasma does `m_fCount += 50.0f * GetFrameTime()` and UpdateWaveProp
	// integrates by GetFrameTime(); at a frame time of ZERO both rewrite the
	// identical values every frame, so the surface is recomputed forever and
	// never moves. Report the clock, on change, so "the update runs" and "the
	// update does something" stay separate questions (§16 was this same family).
	{
		static int s_nOn = -1;
		if (s_nOn < 0) s_nOn = getenv("LT_TRACE_PGFX") ? 1 : 0;
		if (s_nOn)
		{
			const float fFT = g_pGameClientShell->GetFrameTime();
			static float s_fLast = -1.0f;
			static uint32 s_nZero = 0, s_nCalls = 0;
			++s_nCalls;
			if (fFT <= 0.0f) ++s_nZero;
			// ⚠️ Report only what matters: a ZERO frame time, or a slow
			// heartbeat. Reporting every change floods the log with thousands of
			// lines (it changes every frame by construction) and buries the
			// [glpg] height lines you actually want to read.
			//
			// MEASURED AND RULED OUT (2026-07-27, at the C01S01 red bridge):
			// 0 of 4568 calls had frameTime 0, values 0.011-0.028. The clock is
			// healthy; "frame time is zero" is NOT why water can look frozen.
			if (fFT <= 0.0f || (s_nCalls % 2000) == 0)
			{
				fprintf(stderr, "[pgfx] frameTime=%.5f surfaceType=%d "
				                "(%u of %u calls had frameTime 0)%s\n",
				        fFT, (int)m_nSurfaceType, s_nZero, s_nCalls,
				        (fFT <= 0.0f) ? "  <-- ⚠️ ZERO: nothing can move" : "");
				s_fLast = fFT;
			}
		}
	}

	pgfx_Gate("OK: animating");
    return LTTRUE;
}
