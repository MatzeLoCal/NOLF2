...
void SendTriggerMsgToObject(LPBASECLASS pSender, LPBASECLASS pObj, LTBOOL, const char* szMessage)
{
	...
	// Local pointer smuggled through the message; must be full pointer width
	// (the uint32 cast truncated it on LP64). All MID_TRIGGER readers use
	// Readuint64 to match.
	cMsg.Writeuint64((uint64)(uintptr_t)szMessage);
