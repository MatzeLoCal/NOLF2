...
LTBOOL CBaseScreen::Init(int nScreenID)
{
	if (!s_bReadLayout)
	{
		...
		uint8 nSize = (uint8)((LTFLOAT)s_HelpSize * g_pInterfaceResMgr->GetFontRatio());
...
LTBOOL CBaseScreen::CreateTitle(char *lpszTitle)
{
	...
	else
	{
		...
		uint8 nFontSize = (uint8)((LTFLOAT)m_TitleSize * g_pInterfaceResMgr->GetFontRatio());
...
uint32 CBaseScreen::OnCommand(uint32 dwCommand, uintptr_t dwParam1, uintptr_t dwParam2)
{
	switch(dwCommand)
	{
	case CMD_BACK:
