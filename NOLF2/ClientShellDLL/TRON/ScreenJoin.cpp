...
namespace
{
	...
	void JoinLanConfirmationCallback(LTBOOL bReturn, void *pData)
	{
		CScreenJoin *pThisScreen = (CScreenJoin*)g_pInterfaceMgr->GetScreenMgr()->GetScreenFromID(SCREEN_ID_JOIN);
		if (pThisScreen)
			pThisScreen->SendCommand(bReturn ? CMD_OK : CMD_CANCEL,(uintptr_t)pData,CMD_JOIN);
	}
}
...
uint32 CScreenJoin::OnCommand(uint32 dwCommand, uintptr_t dwParam1, uintptr_t dwParam2)
{
	switch(dwCommand)
	{
		case CMD_CANCEL :
