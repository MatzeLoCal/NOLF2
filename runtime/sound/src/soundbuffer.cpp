...
{
    // ★★ LT_MUTE_SOUNDS="substr[;substr...]" — BISECT BY EAR (§76).
    //
    // Refuse to load any sound whose filename contains one of the substrings,
    // case-insensitively. This is the audio twin of LT_WORLD_ONLY: when a noise
    // cannot be identified by reading code, silence candidates one at a time
    // until it goes away, and the one that matters names itself. Matching here,
    // at the single load choke point, means the sound never exists at all — no
    // buffer, no instance, no voice — so there is nothing left to alias with.
    //
    //   LT_MUTE_SOUNDS="waterfall"          just the waterfall
    //   LT_MUTE_SOUNDS="waterfall;jpevening;wind_light"   all the ambience
    //   LT_MUTE_SOUNDS="voice/"             every line of dialogue
    if (fileIdent.m_Filename)
    {
        static const char *s_pMute = getenv("LT_MUTE_SOUNDS");
        if (s_pMute && s_pMute[0])
        {
            char szLower[512];
            uint32 nAt = 0;
            for (const char *p = fileIdent.m_Filename; *p && nAt < sizeof(szLower) - 1; ++p, ++nAt)
                szLower[nAt] = (char)tolower((unsigned char)*p);
            szLower[nAt] = 0;

            char szPats[512];
            strncpy(szPats, s_pMute, sizeof(szPats) - 1);
            szPats[sizeof(szPats) - 1] = 0;
            for (uint32 n = 0; szPats[n]; ++n)
                szPats[n] = (char)tolower((unsigned char)szPats[n]);

            for (char *pTok = strtok(szPats, ";"); pTok; pTok = strtok(NULL, ";"))
            {
                if (pTok[0] && strstr(szLower, pTok))
                {
                    fprintf(stderr, "[snd] MUTED by LT_MUTE_SOUNDS ('%s'): %s\n",
                            pTok, fileIdent.m_Filename);
                    return LT_ERROR;
                }
            }
        }
