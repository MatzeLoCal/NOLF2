...
LTRESULT CSysTexInterface::CreateTextureFromName(HTEXTURE &hTexture, const char *pFilename)
{
	...
	if (pIdent) 
	{
		...
		if (hTexture) 
		{
			...
			{
				// ★★ A FAILED TEXTURE MUST COME BACK AS NULL, NOT AS A FREED
				// POINTER. The bank entry was released here but hTexture was
				// left pointing at it and pIdent->m_pData still cached it, so:
				//
				//  * CInterfaceResMgr::GetTexture (InterfaceResMgr.cpp:600)
				//    IGNORES this function's return code and only tests
				//    `if (hTex)`, so it handed the caller a DANGLING handle;
				//  * the next request for the same name took the
				//    `pIdent->m_pData` fast path above and returned the same
				//    dangling pointer, incrementing a refcount on freed memory.
				//
				// Visible symptom: holstering drew a WHITE SQUARE where the HUD
				// ammo icon goes. WEAPONS.TXT [Ammo44] "Holster" authors
				// Icon = "" (empty, like [Weapon30]'s PVModel), which cannot
				// load; CHUDAmmo::Render does `if (m_hIcon) DrawPrim(...)`
				// (HUDAmmo.cpp:122) and so drew an untextured quad.
				//
				// With hTexture nulled, GetTexture returns NULL, the HUD draws
				// nothing, and the authored "no icon" is honoured.
