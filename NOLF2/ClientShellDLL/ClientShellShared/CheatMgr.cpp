...
// Development only cheats...
	...
	// ★ macOS port addition (not in retail). Encoding is the same as every
	// entry above: Check() maps typed text with ((c ^ 38) + i) ^ 7 and compares
	// against these. Verified round-trip on the retail entries — god -> "FMC",
	// pos -> "QMP", ammo -> "@KJK", maphole -> "LO_VJHN".
	// ⚠️ Index 6 of the encoded form is a literal backslash, hence the "\\".
	{ "VHBP@S\\U`STT^Wj",	LTFALSE },  // whereismymonkey	- show current level name
};
...
bool CCheatMgr::Process( CheatCode nCheatCode, CParsedMsg &cMsg )
{
	...
	switch ( nCheatCode )
	{
		...
		case CHEAT_WHEREAMI:	// show/hide the current level name
			WhereAmI(!s_CheatInfo[nCheatCode].bActive);
		break;
...
// ★ macOS PORT: the development cheats are ENABLED here even though we build
// with _FINAL (macbuild/CMakeLists.txt:19 defines it for every non-Debug
// config). They are the porting effort's own diagnostic tools —
// `mpclip` (spectator flight, the only way to reach a vantage point for a
// reference screenshot), `mpcampos`, `mptriggers`, `mpremoveai`, and
// `budonkadonk` (chase view, the ONLY way to see the player model and so the
// only way to verify §47's render-style alpha op).
//
// ⚠️ THE ARRAY AND THE ENUM ALWAYS CONTAINED THESE — only the HANDLERS were
// compiled out, so `Check()` matched the typed string, `Process()` fell to
// `default: return false`, and the cheat silently did nothing with no message.
// That is a nasty shape: the cheat looks registered and is not.
...
// ----------------------------------------------------------------------- //
//
//	ROUTINE:	CCheatMgr::WhereAmI()
//
//	PURPOSE:	Toggle the on-screen display of the current level name.
//				macOS port addition — so a playtest bug report can name the
//				level it came from without fingerprinting it from a log.
//
// ----------------------------------------------------------------------- //

void CCheatMgr::WhereAmI(LTBOOL bMode)
{
	s_CheatInfo[CHEAT_WHEREAMI].bActive = bMode;

	if (g_pGameClientShell)
	{
		g_pGameClientShell->ShowLevelName((bMode == LTTRUE));
	}

	if (bMode)
	{
		g_pChatMsgs->AddMessage("Show level name enabled.", kMsgCheatConfirm);
	}
	else
	{
		g_pChatMsgs->AddMessage("Show level name disabled.", kMsgCheatConfirm);
	}
}
