...
void ActiveWorldModel::SetPowerOn( )
{
	...
	if( m_nAWMType == AWM_TYPE_ROTATING ) 
	{
		...
		if( (m_dwPropFlags & AWM_PROP_ROTATEAWAY) && m_hActivateObj )
		{
			...
			LTRotation	rTestRot;
			LTVector	vZero(0,0,0);   // CalculateNewPosRot takes a non-const ref

			CalculateNewPosRot( vTestPos1, rTestRot, vZero, m_fPowerOnTime, cf100Percent );
			...
			CalculateNewPosRot( vTestPos2, rTestRot, vZero, m_fPowerOnTime, cf100Percent );
