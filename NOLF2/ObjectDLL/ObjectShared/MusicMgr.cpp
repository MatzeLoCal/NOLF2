...
{
	// A theme need not define any intensity levels for a mood -- the cinematic
	// levels ask for the theme "cinematics", which has no section at all in
	// Attributes\Music.txt, so every m_acMoods entry is 0. m_acMoods is uint32,
	// so the count-1 below asks GetRandom for a number in [0,-1]; its
	// divide-by-zero guard then returns hi == -1 half the time and
	// m_aanMoods[eMood][(uint32)-1] reads 4GB past the array. On 32-bit Windows
	// that wrapped around into the object itself and went unnoticed; on LP64 it
	// is a wild read that killed the first mission load. Nothing authored ==
	// nothing to play. (DoEvent already guards its own count exactly this way.)
	if ( m_acMoods[eMood] == 0 ) return false;
