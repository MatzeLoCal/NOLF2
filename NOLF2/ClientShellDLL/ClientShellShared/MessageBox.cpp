...
LTBOOL CMessageBox::HandleKeyDown(int key, int rep)
{
	...
	if (getenv("LT_TRACE_KEYS"))
		fprintf(stderr, "[keys] CMessageBox::HandleKeyDown key=%d type=%d edit=%p\n",
		        key, (int)m_eType, (void*)m_pEdit);
...
uint32 CMessageBox::OnCommand(uint32 dwCommand, uintptr_t dwParam1, uintptr_t dwParam2)
{
	switch(dwCommand)
	{
	case CMD_OK:
