...
{
	// Textures created through ILTTexInterface::CreateTextureFromData (font
	// atlases, procedural surfaces) deliberately have no file reference —
	// don't dereference m_pFile blindly, callers only want a name to log.
	if (!pTexture || !pTexture->m_pFile)
		return "<no file>";
...
// File-local recursion guard for r_InitRender. (On Win32 this lived in
// g_ClientGlob; that struct is Win32-only and absent on the linuxdsys path, so
// a file-static is the portable equivalent — it tracks the same "am I inside
// r_InitRender" state.)
static BOOL s_bInitializingRenderer = LTFALSE;
LTRESULT r_InitRender(RMode *pMode)
{
	...
	if(s_bInitializingRenderer)
		return LT_OK;


	VarSetter<BOOL> setter(&s_bInitializingRenderer, LTTRUE, LTFALSE);
