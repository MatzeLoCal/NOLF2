#include "bdefs.h"
#ifdef LT_MACOS
#include "ltmacwindow.h"   // LTMacWin_GetNSWindow (Cocoa window handle)
#include "macos_cres.h"    // LTMacCRes_GetDefault (cres.dll string resources)
#endif
	...
	LTWarpPt *pCoords, int nCoords, DrawWarpFn fn)
{
	...
	// The warp lookup tables are indexed by destination Y (the dest coordinates
	// are clipped below to [0, m_Height-1]).  A high-DPI (Retina) screen surface
	// can be far taller than the legacy fixed 1024-entry buffer, so size the
	// tables to the destination height -- indexing past 1024 used to smash the
	// stack.  Keep the stack fast-path for the common (<=1024) case.
	WarpCoords leftStack[1024], rightStack[1024];
	WarpCoords *leftCoords = leftStack, *rightCoords = rightStack;
	std::vector<WarpCoords> leftHeap, rightHeap;
	if (pDest->m_Height > 1024)
	{
		leftHeap.resize(pDest->m_Height);
		rightHeap.resize(pDest->m_Height);
		leftCoords = &leftHeap[0];
		rightCoords = &rightHeap[0];
	}
...
{
	// Always initialize the output so callers never receive an uninitialized
	// value if the hook fails or the requested module isn't bound yet.
	if(pData)
		*pData = NULL;
	...
	{
#ifdef LT_MACOS
		*pData = LTMacWin_GetNSWindow();   // g_ClientGlob is Win32-only; use the Cocoa window
#else
...
#ifdef LT_MACOS
	else if(stricmp(pName, "MacWndProcSlot")==0)
	{
		// There is no Win32 window proc on macOS, so the game cannot hook one --
		// yet a hooked proc is the ONLY route NOLF2 has for mouse input
		// (IClientShell has no mouse callbacks; WM_MOUSEMOVE drives the game's
		// own cursor and WM_LBUTTONDOWN clicks menu items). Hand the game a
		// pointer to our WNDPROC slot instead: CGameClientShell::HookWindow
		// stores HookedWindowProc there, and the Cocoa pump synthesizes the same
		// WM_ messages into it. Going through GetEngineHook keeps libCShell free
		// of engine link symbols (it is a plain data channel over the vtable).
		*pData = (void*)LTMacWin_GetWndProcSlot();
		return LT_OK;
	}
#endif
	...
	{
#ifdef LT_MACOS
		// No PE loading on macOS: hand back our parsed-cres module object
		// (lazily loads cres.dll out of the rez on first use). The game treats
		// this purely as an opaque HINSTANCE and passes it to LoadString().
		*pData = LTMacCRes_GetDefault();
		return (*pData != NULL) ? LT_OK : LT_ERROR;
#else
...
// macOS: keep the console drawable even in the retail-parity (_FINAL) build —
// it is the only in-engine 2D/diagnostic surface until the game UI is ported.
#if !defined(_FINAL) || defined(LT_MACOS)
