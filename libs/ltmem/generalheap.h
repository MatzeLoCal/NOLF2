...
// 64-bit safe: blocks are aligned to GENERALHEAPHEADERSIZE (16) so the low 4
// bits of every block pointer are always zero and are reusable as flag bits.
// (Original packed flags into the *high* bits of a 32-bit pointer, which cannot
// survive on LP64 where pointers are 64 bits.)
#define GENERALHEAPFLAGMASK (~(uintptr_t)0xF)

// flags (stored in the reserved low bits, see mask above)
#define  GENERALHEAPBLOCKFREEFLAG ((uintptr_t)0x1)
...
GENERALHEAPINLINE uint32 CGeneralHeap::GetMemBlockSize(CGeneralHeapHeader* pMemBlock)
{
	...
	{
		return (uint32)((uintptr_t)pMemBlock->GetNextMemBlock() - (uintptr_t)pMemBlock - GENERALHEAPHEADERSIZE);
	}
	else
	{
		return (uint32)((uintptr_t)m_pHeapEnd - (uintptr_t)pMemBlock - GENERALHEAPHEADERSIZE);
	}
}
