...
{
	uint16 nLength;
	*pStream >> nLength;        // (removed (uint16) cast: >> needs an lvalue, not an rvalue cast)
	m_nID = 0;
	for (; nLength; --nLength)
	{
		uint8 nNextChar;
		*pStream >> nNextChar;  // (removed (uint8) cast)
		m_nID *= 31;
...
bool CWorldClientBSP::LoadRenderData(ILTStream *pStream)
{
	...
	// A renderer may not provide a world-data loader (the null/GL bring-up
	// renderer). The render section is the last thing read from the world file
	// (the caller seeks straight to it and nothing reads the stream after us),
	// so it is safe to skip the render blob and the trailing light groups.
	// Phase 2's GL renderer will supply LoadWorldData and restore lighting.
	if (!r_GetRenderStruct()->LoadWorldData)
		return true;
	...
	*pStream >> nNumLightGroups;   // (removed (uint32) cast: >> needs an lvalue)
