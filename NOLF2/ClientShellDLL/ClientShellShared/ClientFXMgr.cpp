...
CBaseFX* CClientFXMgr::CreateFX(const char *sName, FX_BASEDATA *pBaseData, CBaseFXProps* pProps, HOBJECT hInstParent)
{
	...
	// ⚠️ TWO SILENT FAILURE PATHS LIVE IN THIS FUNCTION: an unknown effect TYPE
	// (pFxRef NULL) and an Init() that returns false. Either drops the effect
	// with no diagnostic, which is how a group can half-work — the welder's
	// DynaLight playing while its flame never appears. LT_TRACE_FX=1 reports
	// both.
	static int s_nTraceFX = -1;
	if (s_nTraceFX < 0) s_nTraceFX = getenv("LT_TRACE_FX") ? 1 : 0;

	if( pFxRef )
	{
		pNewFX = pFxRef->m_pfnCreate();
		if (s_nTraceFX && !pNewFX)
			fprintf(stderr, "[fx] CreateFX('%s'): type known but m_pfnCreate returned NULL\n",
			        sName ? sName : "<null>");
	}
	else if (s_nTraceFX)
	{
		fprintf(stderr, "[fx] CreateFX('%s'): UNKNOWN EFFECT TYPE — dropped\n",
		        sName ? sName : "<null>");
	}
	...
	if( pNewFX ) 
	{
		...
		if( !pNewFX->Init(m_pClientDE, pBaseData, pProps) )
		{
			...
			if (s_nTraceFX)
				fprintf(stderr, "[fx] CreateFX('%s'): Init() FAILED — dropped%s\n",
				        sName ? sName : "<null>",
				        pBaseData->m_sNode[0] ? " (respawned as a node instance)" : "");
		...
		{
			fprintf(stderr, "[fx] CreateFX('%s'): OK\n", sName ? sName : "<null>");
		}
	}
