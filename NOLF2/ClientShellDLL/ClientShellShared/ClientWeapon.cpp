...
#if defined(LT_MACOS)
// LT_TRACE_WEAPON=1 — the client-side weapon state machine. Pairs with the
// [wpn] server trace in ObjectDLL/ObjectShared/Weapon.cpp: [cw] shows whether
// the weapon leaves its fire/reload state at all, [wpn] whether the server
// accepted the shot.
static bool ClientWeaponTrace()
{
	static bool s_bTrace = ( getenv( "LT_TRACE_WEAPON" ) != NULL );
	return s_bTrace;
}

static char const *ClientWeaponStateName( WeaponState eState )
{
	switch ( eState )
	{
		case W_INACTIVE:      return "INACTIVE";
		case W_IDLE:          return "IDLE";
		case W_BEGIN_FIRING:  return "BEGIN_FIRING";
		case W_FIRING:        return "FIRING";
		case W_FIRED:         return "FIRED";
		case W_END_FIRING:    return "END_FIRING";
		case W_RELOADING:     return "RELOADING";
		case W_FIRING_NOAMMO: return "FIRING_NOAMMO";
		case W_SELECT:        return "SELECT";
		case W_DESELECT:      return "DESELECT";
		case W_AUTO_SWITCH:   return "AUTO_SWITCH";
		default:              return "?";
	}
}
#endif
...
#if defined(LT_MACOS)
	// Heartbeat while the fire key is held: if the weapon never leaves its
	// fire/reload state the [cw] FIRE KEY line above never appears, and this
	// is what says which animation it is stuck in.
	if ( ClientWeaponTrace() && bFire && m_hObject )
	{
		static LTFLOAT s_fLastBeat = 0.0f;
		LTFLOAT fNow = g_pLTClient->GetTime();
		if ( fNow - s_fLastBeat >= 1.0f )
		{
			s_fLastBeat = fNow;
			uint32 dwAni = g_pLTClient->GetModelAnimation( m_hObject );
			fprintf( stderr, "[cw] held: state=%s ani=%u done=%d loop=%d clip=%d newclip=%d "
			                 "fire=%d fireAni=%d reloadAni=%d postfireAni=%d\n",
			         ClientWeaponStateName( GetState() ), dwAni,
			         (int)!!( g_pLTClient->GetModelPlaybackState( m_hObject ) & MS_PLAYDONE ),
			         (int)g_pLTClient->GetModelLooping( m_hObject ),
			         (int)m_nAmmoInClip, (int)m_nNewAmmoInClip, (int)m_bFire,
			         (int)IsFireAni( dwAni ), (int)IsReloadAni( dwAni ),
			         (int)IsPostFireAni( dwAni ) );
		}
	}
#endif
...
#if defined(LT_MACOS)
		if ( ClientWeaponTrace() )
		{
			fprintf( stderr, "[cw] FIRE KEY -> %s (state=%s clip=%d ammo=%d)\n",
			         ClientWeaponStateName( eRet ), ClientWeaponStateName( GetState() ),
			         (int)m_nAmmoInClip,
			         g_pPlayerStats ? g_pPlayerStats->GetAmmoCount( m_nAmmoId ) : -1 );
		}
#endif
...
#if defined(LT_MACOS)
	if ( ClientWeaponTrace() && eOldState != eNewState )
	{
		fprintf( stderr, "[cw] state %s -> %s (w=%d clip=%d/%d ammo=%d ani=%u done=%d)\n",
		         ClientWeaponStateName( eOldState ), ClientWeaponStateName( eNewState ),
		         (int)m_nWeaponId, (int)m_nAmmoInClip,
		         m_pWeapon ? (int)m_pWeapon->nShotsPerClip : -1,
		         g_pPlayerStats ? g_pPlayerStats->GetAmmoCount( m_nAmmoId ) : -1,
		         m_hObject ? g_pLTClient->GetModelAnimation( m_hObject ) : 0u,
		         m_hObject ? (int)!!( g_pLTClient->GetModelPlaybackState( m_hObject ) & MS_PLAYDONE ) : -1 );
	}
#endif
