...
CGameClientShell::~CGameClientShell()
{
	for (int i=0; i < kMaxDebugStrings; i++)
	{
		...
		if (m_pTopRightDebugString[i])
		{
			g_pFontManager->DestroyPolyString(m_pTopRightDebugString[i]);
			m_pTopRightDebugString[i] = NULL;
		}
	}
...
void CGameClientShell::OnKeyDown(int key, int rep)
{
	...
	// §52: LT_TRACE_KEYS=1 traces the backspace/text-input path. The OS layer
	// is confirmed good (LT_TRACE_INPUT shows mac=51 -> VK 8); this narrows
	// WHERE above OnKeyDown the key is lost.
	if (getenv("LT_TRACE_KEYS"))
		fprintf(stderr, "[keys] CGameClientShell::OnKeyDown key=%d -> InterfaceMgr\n", key);
...
void CGameClientShell::UpdateDebugInfo()
{
	...
	// ★ macOS port addition ("whereismymonkey"): show the current level name,
	// large, in the upper right — so a playtest screenshot names its own level.
	// Recomputed every frame, so it follows level transitions on its own.
	if (m_bShowLevelName && g_pMissionMgr)
	{
		const char* pszWorld = g_pMissionMgr->GetCurrentWorldName();
		if (pszWorld && pszWorld[0])
		{
			// GetCurrentWorldName() is a full path
			// ("Worlds\RetailSinglePlayer\c08s03"); show just the leaf, which
			// is what the level is actually called. Accept either separator —
			// the io layer normalizes them, so both appear in practice.
			const char* pszLeaf = pszWorld;
			for (const char* p = pszWorld; *p; ++p)
			{
				if (*p == '\\' || *p == '/')
					pszLeaf = p + 1;
			}

			SetDebugString(pszLeaf, eDSTopRight, 0);
		}
	}
...
void CGameClientShell::SetDebugString(const char* strMessage, DSSL eLoc, uint8 nLine)
{
	...
	// eDSTopRight is deliberately a FIXED point size rather than the layout's
	// help size — it exists to be legible in a screenshot.
	float fPoints = (eLoc == eDSTopRight) ? (float)kTopRightDebugPointSize
	                                      : (float)g_pLayoutMgr->GetHelpSize();
	float fScaled = fPoints * g_pInterfaceResMgr->GetXRatio();
	if (fScaled > 255.0f) fScaled = 255.0f;   // nSize is uint8
	uint8 nSize = (uint8)fScaled;
	...
	switch (eLoc)
	{
		...
		case eDSTopRight :
		{
			if (!m_pTopRightDebugString[nLine])
			{
				pPolyString = g_pFontManager->CreatePolyString(pFont, "", 0.0f, 0.0f);
				m_pTopRightDebugString[nLine] = pPolyString;
			}
			else
			{
				pPolyString = m_pTopRightDebugString[nLine];
			}
		}
		break;
...
void CGameClientShell::ClearDebugStrings()
{
	for (int i=0; i < kMaxDebugStrings; i++)
	{
		...
		if (m_pTopRightDebugString[i])
		{
			m_pTopRightDebugString[i]->SetText("");
		}
	}
}
...
void CGameClientShell::RenderDebugStrings()
{
	...
	for (int i=0; i < kMaxDebugStrings; i++)
	{
		...
		// Top right, stacking DOWNWARDS from the top edge (the two sets above
		// stack upwards from the bottom). Inset slightly so the text does not
		// touch the screen edge.
		if (m_pTopRightDebugString[i])
		{
			const float kTopRightInset = 8.0f;

			fx = fScreenWidth - m_pTopRightDebugString[i]->GetWidth() - kTopRightInset;
			fy = kTopRightInset + fYTopRightOffset;

			fYTopRightOffset += m_pTopRightDebugString[i]->GetHeight();

			m_pTopRightDebugString[i]->SetPosition(fx, fy);
			m_pTopRightDebugString[i]->Render();
		}
	}
}
...
#ifdef LT_MACOS
	// No Win32 window proc to hook on macOS -- but HookedWindowProc is still the
	// game's ONLY route for mouse input (IClientShell has no mouse callbacks:
	// WM_MOUSEMOVE drives CInterfaceMgr::m_CursorPos, i.e. the game's own cursor
	// sprite, and WM_LBUTTONDOWN clicks menu items). Register it with the engine
	// instead; the Cocoa event pump synthesizes the same messages into it.
	// (The Win32 path below cannot work here anyway: the compat
	// GetWindowLongPtr returns 0, so it would bail out two lines further down.)
	{
		void *pSlot = NULL;
		if(g_pLTClient->GetEngineHook("MacWndProcSlot", &pSlot) != LT_OK || !pSlot)
		{
			TRACE("HookWindow - ERROR - no MacWndProcSlot engine hook!\n");
			return FALSE;
		}
		*(LRESULT (**)(HWND, UINT, WPARAM, LPARAM))pSlot = HookedWindowProc;
		return TRUE;
	}
#endif
