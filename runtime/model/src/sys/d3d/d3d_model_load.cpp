...
// render.h declares r_GetRenderStruct(), which the body below calls on the
// client. On macOS we build the (null) renderer into the client, so include it
// here too rather than taking the server-style default path.
...
CDIModelDrawable *ModelPiece::CreateModelRenderObject( uint32 type )
{
			...
			// A renderer may not provide CreateRenderObject at all (the null/GL
			// bring-up renderer) — fall through to the dummy drawable then.
			if (r_GetRenderStruct()->m_bLoaded && r_GetRenderStruct()->CreateRenderObject)
