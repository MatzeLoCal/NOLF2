...
{
	// ★★★ CLEAR THE OUT-PARAMETER FIRST (§72).
	//
	// The original returned LT_ERROR from the m_nAnims check WITHOUT ever
	// touching hTex. Its only caller — CSpriteSystem::Init, spritesystemfx.cpp:303
	// — passes elements of an array from `debug_newa(HTEXTURE, n)`, i.e. plain
	// `new HTEXTURE[n]`, which for a pointer type leaves every element
	// UNINITIALIZED, and it ignores the return value. CSpriteSystem::Term then
	// does `if (m_pTexArray[i]) ReleaseTextureHandle(m_pTexArray[i])` — garbage
	// is non-null, so it writes a refcount THROUGH a garbage pointer and can
	// then free it. That is the crash the user hit leaving c06s04:
	//   ___BUG_IN_CLIENT_OF_LIBMALLOC_POINTER_BEING_FREED_WAS_NOT_ALLOCATED
	//   LTMemFree <- r_UnloadSystemTexture <- FreeSharedTexture
	//             <- ReleaseTextureHandle  <- CSpriteSystem::Term
	// An out-parameter that is left untouched on a failure path is a defect even
	// when every caller checks the return — and this caller does not.
	hTex = LTNULL;

	Sprite *pSprite = m_pSprite ? m_pSprite->GetSprite() : LTNULL;
	if (!pSprite || !pSprite->m_Anims)
	{
		return LT_ERROR;
	}

	if (iAnim >= pSprite->m_nAnims)
	{
		return LT_ERROR;
	}
	...
	if (pAnim)
	{
		// ⚠️ iFrame was NEVER bounds-checked here, unlike SetCurPos (:34) and
		// GetNumFrames (:15), which both check. Past the end of m_Frames the
		// m_pTex read is whatever follows in the heap — a non-null garbage
		// pointer passes the check below and is refcounted and later freed.
		if (!pAnim->m_Frames || iFrame >= pAnim->m_nFrames)
		{
			return LT_ERROR;
		}
