...
LTRESULT Model::Load(ModelLoadRequest *pRequest, const char* pFilename)
{
	...
	//
	// ⚠️ EXPERIMENT HOOK, default OFF: LT_MODEL_MINVER=<n> lowers this floor.
	// FX\WATERFALL2.LTB (half the C01S01 cascade, §36) is version 18 while this
	// tree requires 23 -- and it is the ONLY copy in the install, so retail's
	// identical check would reject it too. This source tree is later than the
	// NOLF2 retail build (it carries TRON), so the floor may have been raised
	// after the asset was authored. The hook exists to answer "is the v18 layout
	// still parseable, or does the stream desync?" WITHOUT touching game data.
	// Do not ship it on unless the geometry proves correct.
	{
		static int s_nMinVer = -1;
		if (s_nMinVer < 0)
		{
			const char *p = getenv("LT_MODEL_MINVER");
			s_nMinVer = p ? atoi(p) : MIN_MODEL_FILE_VERSION;
		}
		if ((int)m_FileVersion < s_nMinVer)
		{
			char msg[ kErrMsgSize ];
			LTSNPrintF(msg, sizeof(msg), "fileversion %d is not supported.", m_FileVersion );
			setLoadErrorMsg(msg);

			fprintf(stderr, "[model] %s: file version %u < minimum %d\n",
			        pFilename ? pFilename : "<null>", m_FileVersion, s_nMinVer);
			dRet = LT_ERROR;
			goto Error;
		}
		if ((int)MIN_MODEL_FILE_VERSION > s_nMinVer && m_FileVersion < MIN_MODEL_FILE_VERSION)
			fprintf(stderr, "[model] %s: ACCEPTING version %u below the %d floor (LT_MODEL_MINVER)\n",
			        pFilename ? pFilename : "<null>", m_FileVersion, (int)MIN_MODEL_FILE_VERSION);
	}
...
Error:
	...
	dsi_ConsolePrint(error_string);
	// ★ dsi_ConsolePrint goes nowhere on macOS (no engine console), so every
	// model-load failure was mute -- and the caller quietly swaps in a 1-unit
	// models\default.ltb cube. error_val is the exact bail-out point; keep it
	// visible. (This is how FX\WATERFALL2.LTB was found, §36.)
	fprintf(stderr, "[model] load error: %s  error_val=%u (%s)\n",
	        pFilename ? pFilename : "<null>", error_val, g_szErrorMsgMap[error_val]);
#if !defined(__LINUX)
    OutputDebugString(error_string);
#endif
