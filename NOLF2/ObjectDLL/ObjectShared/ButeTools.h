...
class CButeTools
{
	public:
		...
		// rvalue-default overload (same rationale as CButeMgr's: callers pass
		// CARange(...) temporaries, which MSVC bound to the non-const ref).
		static CARange&	GetValidatedRange(CButeMgr& buteMgr, const char* const szTagName,const char* const szLabel, CARange&& Default)
		{ return GetValidatedRange(buteMgr, szTagName, szLabel, static_cast<CARange&>(Default)); }
