...
#include "clientmgr.h"
#include "soundmgr.h"   // GetClientILTSoundMgrImpl() prototype
...
// LTVertexShaderMgr / LTPixelShaderMgr live in the D3D9 renderer (render_a/sys/d3d)
// which isn't built on macOS. Programmable shaders are a Phase 2 (GL/Metal)
// concern, so these report "no shader" for now.
bool CLTClient::AddVertexShader(const char *, int, const uint32 *, uint32, bool) { return false; }
void CLTClient::RemoveVertexShader(int) {}
void CLTClient::RemoveAllVertexShaders() {}
LTVertexShader* CLTClient::GetVertexShader(int) { return NULL; }
bool CLTClient::AddPixelShader(const char *, int, bool) { return false; }
void CLTClient::RemovePixelShader(int) {}
void CLTClient::RemoveAllPixelShaders() {}
LTPixelShader* CLTClient::GetPixelShader(int) { return NULL; }
...
{
#ifndef LT_MACOS
	// g_ClientGlob is Win32-only; console enable/up state is tracked there. On
	// macOS the console isn't wired to it yet (the renderer is the null driver),
