...
// Ensure CRITICAL_SECTION (the <windows.h> shim) and the *full* FileRef
// definition (client_filemgr.h; clientmgr.h only forward-declares it) are in
// scope, since both are used as members of the classes declared below.
#include <windows.h>
...
public:
    ...
    CDStreamOpenQueueList() { m_nNumItems = 0; };  // base CLTBaseList default-constructs
...
public:
    ...
    // The friend CDStreamOpenQueueItem accesses ReduceOpenedItems / the item
    // lists / the critical-section helpers.  On macOS the global <windows.h>
    // shim's EnterCriticalSection (a free function) interferes with friend
    // name lookup into this class, so we widen access to public rather than
    // rely on friendship for the members the item touches.
public:
