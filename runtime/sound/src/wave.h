...
// macOS: pull the Win32 audio types (DWORD/WAVEFORMAT/WAVEFORMATEX) from the
// compat shim, and skip the block below that would redefine them (with a wrong
// 64-bit DWORD). The shim defines _WINDOWS_, which gates the block off.
#include <windows.h>

#if defined(__LINUX) && !defined(_WINDOWS_)
