...
// Server init has half a dozen silent "return LT_ERROR" exits; a failure here
// aborts the whole level load with no clue why (the engine's StartupLocal just
// reports "LoadBinaries failed").  Name the step that actually failed.
#define SERVERINIT_FAIL( step ) \
	do { fprintf( stderr, "[srvinit] FAILED at %s (%s:%d)\n", step, __FILE__, __LINE__ ); \
	     return LT_ERROR; } while( 0 )
...
LTRESULT CGameServerShell::OnServerInitialized()
{
	...
	// Surface bute/attribute load failures here instead of crashing later on a
	// NULL g_p*ButeMgr (the original discarded this result).
	if( !m_pGlobalMgr->Init())
	{
		SERVERINIT_FAIL( "CGlobalServerMgr::Init" );
	}
	...
	if( !m_pServerSaveLoadMgr )
	{
		...
		if( !m_pServerSaveLoadMgr )
		{
			SERVERINIT_FAIL( "new CServerSaveLoadMgr" );
		}
	}
	// If the host provided no GameInfo (e.g. engine bring-up harness starting a
	// world directly), the default-constructed options carry an empty profile
	// name, which CSaveLoadMgr::Init rejects. Fall back to a usable default.
	if( m_ServerGameOptions.m_sProfileName.empty( ))
	{
		m_ServerGameOptions.m_sProfileName = "Player";
		m_ServerGameOptions.m_eGameType = eGameTypeSingle;
	}

	if( !m_pServerSaveLoadMgr->Init( m_ServerGameOptions.m_sProfileName.c_str( ), IsMultiplayerGame( )))
	{
		SERVERINIT_FAIL( "CServerSaveLoadMgr::Init" );
	}
	...
	if( !ScmdServer::Instance( ).Init( ))
		SERVERINIT_FAIL( "ScmdServer::Init" );

	// Initialize the BanIPMgr.
	if( !BanIPMgr::Instance( ).Init( ))
		SERVERINIT_FAIL( "BanIPMgr::Init" );
...
bool CGameServerShell::InitGameType()
{
	...
	if( !m_pServerMissionMgr )
	{
		fprintf( stderr, "[srvinit] InitGameType: no mission mgr for game type %d\n",
			(int)m_ServerGameOptions.m_eGameType );
		return false;
	}

	if( !m_pServerMissionMgr->Init( ))
	{
		fprintf( stderr, "[srvinit] InitGameType: mission mgr Init failed\n" );
		return false;
	}
