...
LTBOOL CInterfaceMgr::Update()
{
	...
	if (getenv("LT_TRACE_GAMESTATE"))
	{
		static int s_lastState = -99;
		if ((int)m_eGameState != s_lastState)
		{
			fprintf(stderr, "[gs] GameState -> %d\n", (int)m_eGameState);
			s_lastState = (int)m_eGameState;
		}
	}
...
{
	// Getting out of GS_LOADINGLEVEL means IsPlayerInWorld() -- four separate
	// conditions, any one of which stalls the loading screen forever with no
	// clue which. Report them once a second under LT_TRACE_GAMESTATE.
	if (getenv("LT_TRACE_GAMESTATE"))
	{
		static float s_fLastReport = -1.0f;
		float fNow = g_pLTClient->GetTime();
		if (fNow - s_fLastReport >= 1.0f)
		{
			s_fLastReport = fNow;
			fprintf(stderr, "[gs] loading: serverLoaded=%d worldLoaded=%d "
				"playerState=%d clientObj=%p\n",
				(int)g_pGameClientShell->IsServerLoaded(),
				(int)g_pGameClientShell->IsWorldLoaded(),
				(int)g_pPlayerMgr->GetPlayerState(),
				(void*)g_pLTClient->GetClientObject());
		}
	}
...
void CInterfaceMgr::UpdateSplashScreenState()
{
	...
	if (getenv("LT_TRACE_SPLASH"))
	{
		static int s_f = 0;
		static double s_lastWall = 0.0;
		struct timeval tv; gettimeofday(&tv, NULL);
		double wall = tv.tv_sec + tv.tv_usec * 1e-6;
		double dWall = s_lastWall ? (wall - s_lastWall) : 0.0;
		s_lastWall = wall;
		if ((++s_f % 60) == 0)
			fprintf(stderr, "[splash] fade=%d curFade=%.3f frameT=%.4f gameTime=%.2f | perFrameWall=%.4f\n",
				(int)m_bScreenFade, m_fCurFadeTime, g_pGameClientShell->GetFrameTime(),
				g_pLTClient->GetTime(), dWall);
	}
	...
	{
		if (getenv("LT_TRACE_SPLASH"))
			fprintf(stderr, "[splash] --> EndSplashScreen() (curFade=%.3f)\n", m_fCurFadeTime);
		EndSplashScreen( );
	}
	else if (getenv("LT_TRACE_SPLASH"))
	{
		static int s_g = 0;
		if ((++s_g % 60) == 0)
			fprintf(stderr, "[splash] tail: fade=%d didFadeOut=%d curFade=%.3f (no EndSplash: need fade=1,didFadeOut=1,curFade<=0)\n",
				(int)m_bScreenFade, (int)bDidFadeOut, m_fCurFadeTime);
	}
}
...
LTBOOL CInterfaceMgr::OnKeyDown(int key, int rep)
{
	...
	// §52 tracing — see GameClientShell::OnKeyDown.
	if (getenv("LT_TRACE_KEYS"))
		fprintf(stderr, "[keys] CInterfaceMgr::OnKeyDown key=%d state=%d msgbox=%d\n",
		        key, (int)m_eGameState, (int)m_MessageBox.IsVisible());
...
LTBOOL CInterfaceMgr::ChangeState(GameState eNewState)
{
	...
	bool bTrace = getenv("LT_TRACE_GAMESTATE") != NULL;
	bool bPre = PreChangeState(eCurState, eNewState) != LTFALSE;
	if (bTrace)
		fprintf(stderr, "[cs] ChangeState %d -> %d : PreChangeState=%d (m_eGameState now %d)\n",
			(int)eCurState, (int)eNewState, (int)bPre, (int)m_eGameState);
	if (bPre)
	{
