...
// (Removed a trailing `#ifndef _MSC_VER #include "node.h"` block: node.h lived in
// the trimmed-out tools/PreProcessor and is only relevant to the preprocessor
// tool, not the engine. The Windows engine build skips it via _MSC_VER and links
// fine, so the engine has no dependency on it.)
...
// g_LocatePointInTree removed: dead code (never instantiated anywhere) that
// referenced the preprocessor tool's `CNode` type, which does not exist in the
// engine (the engine BSP node type is `Node`). MSVC's lazy template lookup never
// checked it; Clang's two-phase lookup rejects the undeclared type at parse.
