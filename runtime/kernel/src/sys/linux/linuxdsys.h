...
// Engine version info. The Win32 build reads the EXE version resource; on macOS
// there is no PE resource so a fixed version is reported (see linuxdsys.cpp).
class LTVersionInfo;
LTRESULT dsi_GetVersionInfo(LTVersionInfo &info);
// Copies/locates a client file to a real path (used by font loading etc.).
