...
bool CSpriteSystem::Init(ILTClient *pClientDE, FX_BASEDATA *pBaseData, const CBaseFXProps *pProps)
{
	...
	if( pControl )
	{
		...
		if( m_nSpriteFrames )
		{
			...
			// ⚠️ `debug_newa` is plain `new HTEXTURE[n]`, and HTEXTURE is a raw
			// pointer — so every element starts as GARBAGE, not NULL. Term()
			// below releases any non-null element, which for garbage means a
			// refcount written through a wild pointer and then a free of it.
			// Clear the array before anything can fail (§72).
			for (uint32 nClear = 0; nClear < m_nSpriteFrames; ++nClear)
			{
				m_pTexArray[nClear] = LTNULL;
			}

			uint32 nFailed = 0;
			for (uint32 i = 0; i < m_nSpriteFrames; i ++)
			{
				// ⚠️ HONOUR THE RESULT. On failure the handle is not ours to
				// release; leaving the slot NULL is what makes Term() skip it.
				if (pControl->GetFrameTextureHandle(m_pTexArray[i], 0, i) != LT_OK)
				{
					m_pTexArray[i] = LTNULL;
					++nFailed;
				}
			}

			// ★ A SPRITE FRAME WITH NO TEXTURE DRAWS AS A WHITE QUAD. That is
			// what the user photographed in c06s04, and before §72 the same
			// slots were garbage rather than NULL — so this is the diagnostic
			// that says whether the white quads and the world-unload crash were
			// one bug or two. Silent until it fires; the handoff's rule is that
			// a silent failure path lies harder.
			if (nFailed)
			{
				fprintf(stderr, "[fx] SPRITE SYSTEM: %u of %u frames have NO texture "
				                "(they will draw as white quads) — sprite '%s'\n",
				        nFailed, m_nSpriteFrames,
				        (GetProps() && GetProps()->m_szFileName[0])
				            ? GetProps()->m_szFileName : "<unnamed>");
			}
		}
	}
