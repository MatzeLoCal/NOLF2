...
#define END_EXTERNC() };
	...
	// ⚠️ _MAX_PATH sizes arrays inside structs shared between the engine and the
	// game modules (PlaySoundInfo::m_szSoundName, ...), so every translation
	// unit MUST agree on it. This branch used to say 256 while
	// macbuild/compat/windows.h said 260, and each TU got whichever header it
	// included first -- see the long note in macbuild/compat/msvc_crt_compat.h,
	// which now defines it once for the whole macOS build.
	#ifndef _MAX_PATH
...
// Wide-string primitives: width-agnostic lt_wcs* (from the macOS CRT shim, correct
// under -fshort-wchar) on macOS; the system wcs* everywhere else.
#ifdef LT_MACOS
	#define LT_WCSLEN  lt_wcslen
	#define LT_WCSCPY  lt_wcscpy
	#define LT_WCSNCPY lt_wcsncpy
	#define LT_WCSNCAT lt_wcsncat
	#define LT_WCSCMP  lt_wcscmp
#else
	#define LT_WCSLEN  wcslen
	#define LT_WCSCPY  wcscpy
	#define LT_WCSNCPY wcsncpy
	#define LT_WCSNCAT wcsncat
	#define LT_WCSCMP  wcscmp
#endif
...
#if !defined(__LINUX) || defined(LT_MACOS)
inline uint32 LTStrLen(const wchar_t* pszStr1)
{
	if(pszStr1)
		return (uint32)LT_WCSLEN(pszStr1);
	return 0;
}
#endif
...
*/
/* The array-vs-pointer test must compare against the POINTER size: on LP64,
   sizeof(char*) is 8, so the original `> 4` sent pointer destinations into the
   bounded branch and silently truncated every such copy to 7 characters. */
#define SAFE_STRCPY(dest, src) {\
    if (sizeof(dest) > sizeof(void*))\
    {\
...
inline void LTStrCpy(wchar_t *pDest, const wchar_t *pSrc, uint32 nBufferChars) 
{
	...
	LT_WCSNCPY(pDest, pSrc, nBufferChars - 1);
	pDest[nBufferChars - 1] = '\0';
}
#endif
...
inline int32 LTStrCmp(const wchar_t* pszStr1, const wchar_t* pszStr2)
{
	...
	return LT_WCSCMP(pszStr1, pszStr2);
}
#endif
