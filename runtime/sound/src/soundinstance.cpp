...
// df_GetRawInfo: the physical file/offset/size behind a resource, which the
// streaming path needs to hand the sound driver a seekable file. Declared in
// the io layer's de_file.h (implemented for macOS in sys/linux/linuxfile.cpp).
...
LTRESULT	CSoundInstance::Init( CSoundBuffer &soundBuffer, PlaySoundInfo &playSoundInfo, uint32 dwOffsetTime )
{
	...
	{ 
		...
		g_pClientMgr->AddToObjectMap((uint16)(uintptr_t)playSoundInfo.m_hSound );
		g_pClientMgr->m_ObjectMap[( uint16 )(uintptr_t)playSoundInfo.m_hSound].m_nRecordType = RECORDTYPE_SOUND;
		g_pClientMgr->m_ObjectMap[( uint16 )(uintptr_t)playSoundInfo.m_hSound].m_pRecordData = this;
	}
...
LTRESULT CSoundInstance::DisconnectFromServer( )
{
	...
	if(( uint16 )(uintptr_t)m_hSound != INVALID_OBJECTID && !( m_dwPlaySoundFlags & PLAYSOUND_CLIENT ))
	{
		g_pClientMgr->ClearObjectMapEntry((uint16)(uintptr_t)m_hSound);
		m_hSound = ( HLTSOUND )INVALID_OBJECTID;
	}
...
LTRESULT CSoundInstance::AcquireSample( )
{
	...
	// ★ WHICH SOUND IS THRASHING? (§74) The driver-side trace can only report
	// "118344 frames @22050 Hz" because a voice does not know its filename —
	// and a sample re-acquired hundreds of times a second is the whole
	// complaint. This is the one place that has both. Reported per FILE on a
	// rising threshold, never per call: the storm is thousands of lines and the
	// question is only WHICH file and ROUGHLY how often.
	if ( getenv( "LT_TRACE_SOUND" ) && m_pSoundBuffer )
	{
		const FileIdentifier *pIdent = m_pSoundBuffer->GetFileIdent( );
		const char *pszName = ( pIdent && pIdent->m_Filename ) ? pIdent->m_Filename : "<unnamed>";

		// Small fixed table: the offenders are few, and this must not allocate.
		struct SAcqCount { const char *m_pszName; uint32 m_nCount; uint32 m_nNextReport; };
		static SAcqCount s_aCounts[32];
		static uint32    s_nCounts = 0;

		SAcqCount *pSlot = NULL;
		for ( uint32 n = 0; n < s_nCounts; ++n )
		{
			// Filenames are interned in the file tree, so pointer equality holds
			// for the same file and a strcmp per acquisition is not needed.
			if ( s_aCounts[n].m_pszName == pszName )
			{
				pSlot = &s_aCounts[n];
				break;
			}
		}
		if ( !pSlot && s_nCounts < 32 )
		{
			pSlot = &s_aCounts[s_nCounts++];
			pSlot->m_pszName = pszName;
			pSlot->m_nCount = 0;
			pSlot->m_nNextReport = 25;
		}
		if ( pSlot && ++pSlot->m_nCount >= pSlot->m_nNextReport )
		{
			pSlot->m_nNextReport *= 2;
			fprintf( stderr, "[snd] sample RE-ACQUIRED %u times: %s\n",
			         pSlot->m_nCount, pszName );
		}

		// ★★ ONCE PER FILE: the FORMAT the engine ended up with, next to the
		// NAME (§75). This is the only line that can catch a decompressed
		// sound coming out the wrong LENGTH — compare it against the container:
		// an IMA-ADPCM file's true length is blocks * samplesPerBlock, and if
		// the engine's frame count is larger, the excess is uninitialised memory
		// being mixed as audio. On a LOOPING ambient that is periodic static.
		if ( pSlot && pSlot->m_nCount == 1 )
		{
			WAVEFORMATEX *pWF = m_pSoundBuffer->GetWaveFormat( );
			const uint32 nBytes = m_pSoundBuffer->GetSoundDataLen( );
			if ( pWF )
			{
				const uint32 nAlign = pWF->nBlockAlign ? pWF->nBlockAlign : 1;
				fprintf( stderr, "[snd] BUFFER %s: tag=%u %uch %uHz %ubit  %u bytes"
				                 " -> %u frames (%.2fs)  compressed=%d\n",
				         pszName, (unsigned)pWF->wFormatTag, (unsigned)pWF->nChannels,
				         (unsigned)pWF->nSamplesPerSec, (unsigned)pWF->wBitsPerSample,
				         nBytes, nBytes / nAlign,
				         pWF->nSamplesPerSec ? (float)( nBytes / nAlign ) / (float)pWF->nSamplesPerSec : 0.0f,
...
LTRESULT CSoundInstance::Acquire3DSample( )
{
		...
		( CSoundBuffer* )GetSoundSys()->Get3DUserData( m_h3DSample, SAMPLE_BUFFER ) != m_pSoundBuffer )
	{
		...
		GetSoundSys()->Set3DUserData( m_h3DSample, SAMPLE_BUFFER, (LTSOUNDUSERDATA)(uintptr_t)m_pSoundBuffer );
	}
...
LTRESULT CSoundInstance::AcquireStream( )
{
	...
	if( !m_hStream )
	{
		...
		// NOTE (macOS): this used to be #ifdef'd out because df_GetRawInfo did
		// not exist in the POSIX io layer, which silently killed EVERY streamed
		// sound -- i.e. all of NOLF2's dialogue. Lip-sync ran (it is driven by
		// timing data, not by the audio) so it looked like a mixing bug rather
		// than a file-layer gap. df_GetRawInfo is now implemented in
		// sys/linux/linuxfile.cpp.
