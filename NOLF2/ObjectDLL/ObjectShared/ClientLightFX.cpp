...
#if defined(LT_MACOS)
	// LT_TRACE_LIGHTFX=1 -- the SERVER end of the dynamic-light chain. Counting
	// these tells us how many ClientLightFX objects the level actually has,
	// which is the first thing to establish: if it is 0, the lamp pools are not
	// dynamic lights at all and the renderer has nothing to draw by design.
	if (getenv("LT_TRACE_LIGHTFX"))
	{
		static int s_nCount = 0;
		LTVector vPos;
		g_pLTServer->GetObjectPos(m_hObject, &vPos);
		fprintf(stderr, "[lfx-srv] #%d SendEffectMessage at (%.0f %.0f %.0f) "
		                "color=(%.0f %.0f %.0f) flags=0x%X radius=%.0f..%.0f\n",
		        ++s_nCount, vPos.x, vPos.y, vPos.z,
		        m_vColor.x, m_vColor.y, m_vColor.z, m_dwLightFlags,
		        m_fRadiusMin, m_fRadiusMax);
	}
#endif
