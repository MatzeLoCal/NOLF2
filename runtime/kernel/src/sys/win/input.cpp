#ifdef LT_MACOS
// ================= macOS input (NSEvent-backed) ===================
// The original DirectInput implementation is preserved below (#else).
//
// This implements the engine's InputMgr on top of macos_input.mm's Cocoa
// capture. The engine's model:
//
//   AddAction("Forward", 0)            -- name an action + its command code
//   EnableDevice("##keyboard")         -- turn a device on
//   AddBinding("##keyboard","##17","Forward",lo,hi)
//                                      -- "##17" is a PC DIK scancode (W)
//   ScaleTrigger("##mouse","##x-axis", 0.00125, ...)
//
// ReadInput() then samples live device state and fills:
//   pActionsOn[code] = 1   for each held binding (code >= 0)
//   axisOffsets[-code-1]   for axis actions (AddAction Axis1 -1 -> index 0)
//
// Mouse object ids follow DirectInput: 0/1/2 = x/y/z axes, 3+ = buttons.
// All of this is driven by the retail autoexec.cfg, unmodified.
#include "bdefs.h"
...
struct LTMacBinding
{
	...
	int    m_nActionCode;    // resolved from m_sAction
	int    m_nObject;        // DIK scancode, or mouse object id; -1 = named axis
	int    m_nAxis;          // 0=x 1=y 2=z for axis triggers, else -1
	float  m_fRangeLow, m_fRangeHigh;
	float  m_fScale;
};

std::vector<LTMacAction>  g_aActions;
std::vector<LTMacBinding> g_aBindings;
std::vector<std::string>  g_aEnabledDevices;

bool im_IsKeyboard(const char *p) { return p && stricmp(p, "##keyboard") == 0; }
bool im_IsMouse(const char *p)    { return p && stricmp(p, "##mouse") == 0; }

int im_FindActionCode(const char *pName)
{
	for (size_t i = 0; i < g_aActions.size(); ++i)
		if (stricmp(g_aActions[i].m_sName.c_str(), pName) == 0)
			return g_aActions[i].m_nCode;
	return 0x7FFFFFFF;   // not found
}

// "##17" -> 17. Returns -1 for named triggers like "##x-axis".
int im_ParseObject(const char *pTrigger)
{
	if (!pTrigger) return -1;
	const char *p = pTrigger;
	while (*p == '#') ++p;
	if (*p < '0' || *p > '9') return -1;
	return atoi(p);
}

int im_ParseAxis(const char *pTrigger)
{
	if (!pTrigger) return -1;
	if (strstr(pTrigger, "x-axis")) return 0;
	if (strstr(pTrigger, "y-axis")) return 1;
	if (strstr(pTrigger, "z-axis")) return 2;
	return -1;
}

bool      im_Init(InputMgr*, ConsoleState*) { return true; }
void      im_Term(InputMgr*) { g_aActions.clear(); g_aBindings.clear(); g_aEnabledDevices.clear(); }
bool      im_IsInitted(InputMgr*) { return true; }
void      im_ListDevices(InputMgr*) {}
long      im_PlayJoystickEffect(InputMgr*, const char*, float, float) { return 0; }
bool      im_FlushInputBuffers(InputMgr*) { LTMacInput_ClearAll(); return true; }
LTRESULT  im_ClearInput() { LTMacInput_ClearAll(); return LT_OK; }

void im_AddAction(InputMgr*, const char *pActionName, int actionCode)
{
	if (!pActionName) return;
	LTMacAction cAction;
	cAction.m_sName = pActionName;
	cAction.m_nCode = actionCode;
	g_aActions.push_back(cAction);
}

bool im_EnableDevice(InputMgr*, const char *pDeviceName)
{
	if (!pDeviceName) return false;
	for (size_t i = 0; i < g_aEnabledDevices.size(); ++i)
		if (stricmp(g_aEnabledDevices[i].c_str(), pDeviceName) == 0)
			return true;
	g_aEnabledDevices.push_back(pDeviceName);
	return true;
}

bool im_IsDeviceEnabled(const char *pDeviceName)
{
	if (!pDeviceName) return false;
	for (size_t i = 0; i < g_aEnabledDevices.size(); ++i)
		if (stricmp(g_aEnabledDevices[i].c_str(), pDeviceName) == 0)
...
bool im_ClearBindings(InputMgr*, const char *pDeviceName, const char *pTriggerName)
{
	for (size_t i = 0; i < g_aBindings.size(); )
	{
		bool bMatch = (!pDeviceName || stricmp(g_aBindings[i].m_sDevice.c_str(), pDeviceName) == 0) &&
		              (!pTriggerName || stricmp(g_aBindings[i].m_sTrigger.c_str(), pTriggerName) == 0);
		if (bMatch) g_aBindings.erase(g_aBindings.begin() + i);
		else        ++i;
	}
	return true;
}

bool im_AddBinding(InputMgr*, const char *pDeviceName, const char *pTriggerName,
                   const char *pActionName, float rangeLow, float rangeHigh)
{
	if (!pDeviceName || !pTriggerName || !pActionName) return false;
	LTMacBinding cBind;
	cBind.m_sDevice  = pDeviceName;
	cBind.m_sTrigger = pTriggerName;
	cBind.m_sAction  = pActionName;
	cBind.m_nActionCode = im_FindActionCode(pActionName);
	cBind.m_nObject  = im_ParseObject(pTriggerName);
	cBind.m_nAxis    = im_ParseAxis(pTriggerName);
	cBind.m_fRangeLow  = rangeLow;
	cBind.m_fRangeHigh = rangeHigh;
	cBind.m_fScale     = 1.0f;
	g_aBindings.push_back(cBind);
	return true;
}

bool im_ScaleTrigger(InputMgr*, const char *pDeviceName, const char *pTriggerName,
                     float scale, float, float, float)
{
	for (size_t i = 0; i < g_aBindings.size(); ++i)
	{
		if (stricmp(g_aBindings[i].m_sDevice.c_str(), pDeviceName) == 0 &&
		    stricmp(g_aBindings[i].m_sTrigger.c_str(), pTriggerName) == 0)
			g_aBindings[i].m_fScale = scale;
	}
	return true;
}

// The heart of it: sample devices -> actions + axis offsets.
void im_ReadInput(InputMgr*, BYTE *pActionsOn, float axisOffsets[3])
{
	if (axisOffsets) axisOffsets[0] = axisOffsets[1] = axisOffsets[2] = 0.0f;

	// Mouse motion is a delta accumulated since the previous frame; consume it
	// once here so every binding this frame sees the same value.
	float fMouseDX = 0.0f, fMouseDY = 0.0f;
	LTMacInput_ConsumeMouseDelta(&fMouseDX, &fMouseDY);

	bool bKeyboard = im_IsDeviceEnabled("##keyboard");
	bool bMouse    = im_IsDeviceEnabled("##mouse");

	// LT_TRACE_INPUT=1: one-shot binding census + every action as it fires.
	static int s_nTrace = -1;
	if (s_nTrace < 0)
	{
		s_nTrace = getenv("LT_TRACE_INPUT") ? 1 : 0;
		if (s_nTrace)
			fprintf(stderr, "[in] InputMgr: %u actions, %u bindings, keyboard=%d mouse=%d\n",
			        (unsigned)g_aActions.size(), (unsigned)g_aBindings.size(),
			        (int)bKeyboard, (int)bMouse);
	}

	for (size_t i = 0; i < g_aBindings.size(); ++i)
	{
		const LTMacBinding &cBind = g_aBindings[i];
		if (cBind.m_nActionCode == 0x7FFFFFFF)
			continue;

		if (bKeyboard && im_IsKeyboard(cBind.m_sDevice.c_str()))
		{
			if (cBind.m_nObject >= 0 && LTMacInput_IsDIKDown((unsigned int)cBind.m_nObject))
			{
				if (cBind.m_nActionCode >= 0 && pActionsOn)
					pActionsOn[cBind.m_nActionCode] = 1;
				if (s_nTrace)
					fprintf(stderr, "[in] action '%s' (code %d) ON via %s\n",
					        cBind.m_sAction.c_str(), cBind.m_nActionCode,
					        cBind.m_sTrigger.c_str());
			}
		}
		else if (bMouse && im_IsMouse(cBind.m_sDevice.c_str()))
		{
			if (cBind.m_nAxis == 0 || cBind.m_nAxis == 1)
			{
				float fValue = (cBind.m_nAxis == 0) ? fMouseDX : fMouseDY;
				if (cBind.m_fRangeLow == 0.0f && cBind.m_fRangeHigh == 0.0f)
				{
					// Continuous axis -> axis offset (Axis1 = -1 -> index 0).
					if (cBind.m_nActionCode < 0 && axisOffsets)
					{
						int nIndex = -cBind.m_nActionCode - 1;
						if (nIndex >= 0 && nIndex < 3)
							axisOffsets[nIndex] += fValue * cBind.m_fScale;
						if (s_nTrace && fValue != 0.0f)
							fprintf(stderr, "[in] axis '%s' %.2f -> offset[%d] %.4f\n",
							        cBind.m_sTrigger.c_str(), fValue, nIndex,
							        fValue * cBind.m_fScale);
					}
				}
				else if (fValue >= cBind.m_fRangeLow && fValue <= cBind.m_fRangeHigh)
				{
					if (cBind.m_nActionCode >= 0 && pActionsOn)
						pActionsOn[cBind.m_nActionCode] = 1;
				}
			}
			else if (cBind.m_nObject >= 3)
			{
				// DirectInput mouse objects: 0/1/2 axes, buttons from 3 up.
				if (LTMacInput_IsMouseButtonDown((unsigned int)(cBind.m_nObject - 3)))
				{
					if (cBind.m_nActionCode >= 0 && pActionsOn)
						pActionsOn[cBind.m_nActionCode] = 1;
				}
...
LTRESULT input_GetManager(InputMgr **pMgr) {
    static InputMgr s_mgr;
    static bool s_init = false;
    if (!s_init) {
        s_mgr.Init = im_Init; s_mgr.Term = im_Term; s_mgr.IsInitted = im_IsInitted;
        s_mgr.ListDevices = im_ListDevices; s_mgr.PlayJoystickEffect = im_PlayJoystickEffect;
        s_mgr.ReadInput = im_ReadInput; s_mgr.FlushInputBuffers = im_FlushInputBuffers;
        s_mgr.ClearInput = im_ClearInput; s_mgr.AddAction = im_AddAction;
        s_mgr.EnableDevice = im_EnableDevice; s_mgr.ClearBindings = im_ClearBindings;
        s_mgr.AddBinding = im_AddBinding; s_mgr.ScaleTrigger = im_ScaleTrigger;
        s_mgr.GetDeviceBindings = im_GetDeviceBindings; s_mgr.FreeDeviceBindings = im_FreeDeviceBindings;
        s_mgr.StartDeviceTrack = im_StartDeviceTrack; s_mgr.TrackDevice = im_TrackDevice;
        s_mgr.EndDeviceTrack = im_EndDeviceTrack; s_mgr.GetDeviceObjects = im_GetDeviceObjects;
        s_mgr.FreeDeviceObjects = im_FreeDeviceObjects; s_mgr.GetDeviceName = im_GetDeviceName;
        s_mgr.GetDeviceObjectName = im_GetDeviceObjectName; s_mgr.IsDeviceEnabled = im_IsDeviceEnabled;
        s_mgr.ShowDeviceObjects = im_ShowDeviceObjects; s_mgr.ShowInputDevices = im_ShowInputDevices;
        s_init = true;
    }
    if (pMgr) *pMgr = &s_mgr;
    return LT_OK;
