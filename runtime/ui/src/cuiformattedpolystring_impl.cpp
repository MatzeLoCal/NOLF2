...
void CUIFormattedPolyString_Impl::Parse()
{
	...
	// Index of the first character on the current line.  A wrap must never
	// fire when the line holds only this first character: a single glyph that
	// is by itself wider than the wrap width cannot be wrapped, and forcing an
	// artificial break there stored a word-end of (i-1)==-1 (0xFFFF as uint16)
	// and re-processed the same too-wide glyph forever, saturating the word/
	// line tables with garbage indices -> ApplyXYZ then wrote m_pPolys far out
	// of bounds and smashed the heap.
	int32 nLineStartIdx = 0;
	...
	for (i=0; i<m_Length; i++) 
	{
		...
		// Only wrap when the current line holds more than its first character
		// (i > nLineStartIdx).  Wrapping a line that contains only one glyph is
		// impossible and used to underflow the word-end index; see nLineStartIdx.
		if ( m_WrapWidth  && (line_width >= m_WrapWidth) && (i > nLineStartIdx) ) {
			...
			// The next line begins at the character we are about to (re)process.
			nLineStartIdx = i + 1;
		}
		...
		if (bNewline) {
			...
			// The next character starts a fresh line.
			nLineStartIdx = i + 1;
		}
		...
		switch (align) {
			...
			case CUI_HALIGN_JUSTIFY:
				...
				for (i=line_start; i<=stop_idx; i++) {
					...
					// Defense in depth: there is exactly one poly per character,
					// so an index outside the allocation can only be a layout
					// bug -- never write past m_pPolys.
					if (i < 0 || (uint32)i >= m_NumAllocatedPolys)
						continue;
