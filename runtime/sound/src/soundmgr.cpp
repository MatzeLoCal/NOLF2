...
#ifndef LT_MACOS
// g_ClientGlob is the Win32-only client global (sys/win/dsys_interface.h); it is
// never defined on macOS, where the client uses the linuxdsys interface. The
// named-sound-driver path that reads it (in SoundSys below) is skipped there, so
// macOS falls through to the default sound driver.
...
ILTSoundSys* SoundSys(bool bTerminate)
{
    ...
    else if (!bTerminate && g_pSoundSys == LTNULL)
    {
        ...
        {
				// No sound driver registers a factory on macOS yet (audio is a
				// later phase; see s_nul/OpenAL), so GetSoundFactory() is NULL
				// here — calling through it is a jump-to-0. Callers already
				// null-check the returned sound system ("this could be called
				// before sound is initialized"), so degrade to "no sound".
				ILTSoundFactory* pSoundFactory = ILTSoundFactory::GetSoundFactory();
				if (pSoundFactory != LTNULL)
				{
					// grab the default snddrv.dll NOTE: does't call Init on it.
					g_pSoundSys = pSoundFactory->MakeSoundSystem(LTNULL);
				}
		  }
...
LTRESULT CSoundMgr::Create3DSamples()
{
    ...
    if (m_nMax3DSamples > 0)
    {
        ...
        for (nSample = 0; nSample < m_nMax3DSamples; nSample++)
        {
            ...
            g_pSoundSys->Set3DUserData(m_p3DSampleList[nSample].m_h3DSample, SAMPLE_LISTITEM, (LTSOUNDUSERDATA)(uintptr_t)&m_p3DSampleList[nSample]);
...
LTRESULT CSoundMgr::CreateSWSamples()
{
    ...
    if (m_nMaxSWSamples > 0)
    {
        ...
        for (nSample = 0; nSample < m_nMaxSWSamples; nSample++)
        {
            ...
            g_pSoundSys->SetSampleUserData(m_pSWSampleList[nSample].m_hSample, SAMPLE_LISTITEM, (LTSOUNDUSERDATA)(uintptr_t)&m_pSWSampleList[nSample]);
...
LTRESULT CSoundMgr::Update()
{
    ...
    // ★★ LT_LISTENER_POS="x y z" — PUT THE EAR WHERE THE BUG IS (§75).
    //
    // A headless `+runworld` run always stands at the level start, so every
    // sound measurement it takes is a measurement of the level start. That is
    // how §74 came to "verify" a crackle fix at a spot where the crackle does
    // not happen. This overrides the listener position ONLY — no teleport, no
    // gameplay change — so the earshot, priority and attenuation logic all run
    // as if the player were standing at the reported coordinates.
    //
    // ⚠️ Diagnostic only. It does not move the camera, so the picture will not
    // match the sound; that is the point, not a bug.
    {
        static int      s_nOverride = -1;
        static LTVector s_vOverride;
        if (s_nOverride < 0)
        {
            s_nOverride = 0;
            const char *pEnv = getenv("LT_LISTENER_POS");
            if (pEnv && sscanf(pEnv, "%f %f %f", &s_vOverride.x, &s_vOverride.y, &s_vOverride.z) == 3)
            {
                s_nOverride = 1;
                fprintf(stderr, "[snd] LISTENER PINNED to (%.0f %.0f %.0f)\n",
                        s_vOverride.x, s_vOverride.y, s_vOverride.z);
            }
        }
        if (s_nOverride > 0)
            m_vListenerPosition = s_vOverride;
    }
	...
	// ★ THE DECISIVE NUMBERS FOR THE STEAL-THRASH (§74): how many instances want
	// to be heard versus how many sample slots exist. If EARSHOT exceeds the
	// pools, the loop above steals every frame — Silence() + AcquireSample()
	// restarts a sound at position 0, so a starved ambient loop is re-triggered
	// ~20x/second and you hear its attack, not the sound. Reported on CHANGE, so
	// it is a handful of lines rather than one per frame.
	if ( getenv( "LT_TRACE_SOUND" ) )
	{
		uint32 nEarshot = 0, nWithChannel = 0;
		for ( uint32 dwCount = 0; dwCount < m_dwNumSoundInstances; dwCount++ )
		{
			CSoundInstance *pInst = m_SoundInstanceList[dwCount];
			if ( !pInst || (pInst->GetSoundInstanceFlags() & SOUNDINSTANCEFLAG_DONE) )
				continue;
			if ( pInst->GetSoundInstanceFlags() & SOUNDINSTANCEFLAG_EARSHOT )
				nEarshot++;
			if ( pInst->Get3DSample() || pInst->GetSample() || pInst->GetStream() )
				nWithChannel++;
		}
		// ★★ LT_TRACE_3D=1 — WHERE EACH LIVE SOUND ACTUALLY IS (§77).
		//
		// With 3D sound on, distance attenuation moved from the ENGINE into the
		// driver, and it is computed from the voice's own m_vPos. So "a sound is
		// too loud in the wrong place" is answerable only by printing, per
		// instance: the FILE, the position the engine believes it is at, and the
		// distance to the listener. A voice carrying a stale position from a
		// previous sound shows up here immediately as a distance that does not
		// match where the object was authored (LT_TRACE_SOUNDFX prints that).
		if ( getenv( "LT_TRACE_3D" ) )
		{
			static uint32 s_nTick = 0;
			if ( ( ++s_nTick % 120 ) == 0 )
			{
				fprintf( stderr, "[3d] listener @(%.0f %.0f %.0f)\n",
				         m_vListenerPosition.x, m_vListenerPosition.y, m_vListenerPosition.z );
				for ( uint32 dwC = 0; dwC < m_dwNumSoundInstances; dwC++ )
				{
					CSoundInstance *pInst = m_SoundInstanceList[dwC];
					if ( !pInst || !pInst->GetSoundBuffer( ) )
						continue;
					if ( !pInst->Get3DSample( ) && !pInst->GetSample( ) && !pInst->GetStream( ) )
						continue;
					const FileIdentifier *pI = pInst->GetSoundBuffer( )->GetFileIdent( );
					LTVector vP;
					pInst->Get3DSamplePosition( vP );
					fprintf( stderr, "[3d]   %-34s %s @(%6.0f %6.0f %6.0f) dist=%7.0f\n",
					         ( pI && pI->m_Filename ) ? pI->m_Filename : "?",
					         pInst->Get3DSample( ) ? "3D" : ( pInst->GetStream( ) ? "ST" : "2D" ),
					         vP.x, vP.y, vP.z,
					         ( vP - m_vListenerPosition ).Mag( ) );
				}
			}
		}

		static uint32 s_nLastEarshot = 0xFFFFFFFF, s_nLastChannel = 0xFFFFFFFF;
		if ( nEarshot != s_nLastEarshot || nWithChannel != s_nLastChannel )
		{
			s_nLastEarshot = nEarshot; s_nLastChannel = nWithChannel;
			fprintf( stderr, "[snd] earshot=%u  with-channel=%u  pools: sw=%u 3d=%u%s\n",
			         nEarshot, nWithChannel,
			         (uint32)m_nMaxSWSamples, (uint32)m_nMax3DSamples,
			         ( nEarshot > (uint32)m_nMaxSWSamples + (uint32)m_nMax3DSamples )
			             ? "   <-- OVERSUBSCRIBED, sounds will be stolen and restarted" : "" );
		}
	}
    ...
    for (dwIndex = 0; dwIndex < m_dwNumSoundInstances; dwIndex++)
    {
        ...
        if (!pSoundInstance->UpdateTimer(dwFrameTime))
        {
            ...
            {
                cNewSoundUpdatePacket.Writeuint16((uint16)(uintptr_t)pSoundInstance->GetHSoundDE());
            }
        }
    }
...
LTRESULT CSoundMgr::LinkSampleSoundInstance(HSAMPLE hSample, CSoundInstance *pSoundInstance)
{
    ...
    g_pSoundSys->SetSampleUserData(hSample, SAMPLE_INSTANCE, (LTSOUNDUSERDATA)(uintptr_t)pSoundInstance);
...
LTRESULT CSoundMgr::Link3DSampleSoundInstance(H3DSAMPLE h3DSample, CSoundInstance *pSoundInstance)
{
    ...
    g_pSoundSys->Set3DUserData(h3DSample, SAMPLE_INSTANCE, (LTSOUNDUSERDATA)(uintptr_t)pSoundInstance);
...
LTRESULT CSoundMgr::LinkStreamSoundInstance(HSTREAM hStream, CSoundInstance *pSoundInstance)
{
    ...
    g_pSoundSys->SetStreamUserData(hStream, SAMPLE_INSTANCE, (LTSOUNDUSERDATA)(uintptr_t)pSoundInstance);
