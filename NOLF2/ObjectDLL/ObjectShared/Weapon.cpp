...
#if defined(LT_MACOS)
	// LT_TRACE_WEAPON=1 — the server side of every fire request: the rate
	// limiter's inputs (which silently eat a shot AND its ammo), the clip and
	// the ammo pool. See also the [cw] client-side trace in ClientWeapon.cpp.
	static const bool s_bTraceWeapon = (getenv("LT_TRACE_WEAPON") != NULL);
	if (s_bTraceWeapon)
	{
		fprintf(stderr, "[wpn] Fire w=%d a=%d ts=%u last=%u delta=%u delay=%u "
		                "clip=%d ammo=%d type=%d%s\n",
		        (int)m_nWeaponId, (int)m_nAmmoId, info.nFireTimestamp,
		        m_nLastTimestamp, (uint32)(info.nFireTimestamp - m_nLastTimestamp),
		        m_pWeaponData->m_nFireDelay, (int)m_nAmmoInClip, nAmmo, (int)eAmmoType,
		        ((info.nFireTimestamp - m_nLastTimestamp) < m_pWeaponData->m_nFireDelay)
		            ? "  >>> SWALLOWED BY RATE LIMIT" : "");
	}
#endif
	...
	if (nAmmo > 0)
	{
		...
		if (m_nAmmoInClip > 0)
		{
			...
			for (int i=0; i < nVectorsPerShot; i++)
			{
				...
				if (eAmmoType == PROJECTILE)
				{
					LTBOOL bMade = CreateProjectile(rRot, info);
#if defined(LT_MACOS)
					if (s_bTraceWeapon)
						fprintf(stderr, "[wpn]   CreateProjectile(%s) -> %d  pos=(%.0f %.0f %.0f)\n",
						        m_pAmmoData->pProjectileFX ? m_pAmmoData->pProjectileFX->szClass : "<none>",
						        (int)bMade, info.vFirePos.x, info.vFirePos.y, info.vFirePos.z);
#endif
					if (!bMade)
					{
						return W_IDLE;
