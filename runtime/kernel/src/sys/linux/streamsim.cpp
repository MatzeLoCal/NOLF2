...
// The engine and game name files with Windows-style backslash paths
// ("Save\Player\SinglePlayer\Kpalv.sav"); POSIX needs forward slashes. Without
// this the whole path collapses into one flat filename in the CWD -- which is
// where the stray "Profiles\Player.txt" in the game directory came from.
// (Mirrors df_NormalizeSeparators in the io layer, runtime/kernel/io/src/sys/
// linux/linuxfile.cpp.)
static void ss_NormalizePath(const char *pIn, char *pOut, size_t nOutSize)
{
	size_t i = 0;
	for (; pIn[i] && i + 1 < nOutSize; ++i)
		pOut[i] = (pIn[i] == '\\') ? '/' : pIn[i];
	pOut[i] = '\0';
}
...
ILTStream* streamsim_Open(const char *pFilename, const char *pAccess)
{
	...
	char szPath[1024];
	ss_NormalizePath(pFilename, szPath, sizeof(szPath));

	fp = fopen(szPath, pAccess);
	if(!fp)
		return 0;
