...
// Legacy code uses these stream names unqualified (old <iostream.h> placed them
// in the global namespace; modern <iostream> scopes them to std).
using std::istream;   using std::ostream;     using std::iostream;
using std::ifstream;  using std::ofstream;    using std::fstream;
using std::istrstream; using std::ostrstream; using std::strstream;
using std::streamsize; using std::ios;        using std::endl;
using std::streamoff; using std::strstreambuf; using std::streambuf;
...
public:
	...
	// rvalue-default overloads: game code pervasively passes temporaries as the
	// default (GetVector(tag, att, CAVector(0,0,0))), which MSVC bound to the
	// non-const ref above. Bind the temporary here and forward; it lives to the
	// end of the caller's full expression, so the returned reference is valid
	// exactly as it was under MSVC.
	CRect&    GetRect  (const char* t, const char* a, CRect&& d)    { return GetRect  (t, a, static_cast<CRect&>(d)); }
	CPoint&   GetPoint (const char* t, const char* a, CPoint&& d)   { return GetPoint (t, a, static_cast<CPoint&>(d)); }
	CAVector& GetVector(const char* t, const char* a, CAVector&& d) { return GetVector(t, a, static_cast<CAVector&>(d)); }
	CARange&  GetRange (const char* t, const char* a, CARange&& d)  { return GetRange (t, a, static_cast<CARange&>(d)); }
...
#if defined(LT_MACOS)
// On POSIX '\' is an ordinary filename character, not a path separator, but the
// NOLF config/asset paths are authored with Windows '\' separators.  The engine
// file system normalizes these, but ButeMgr's file-based Parse/Save open the
// path directly through the C++ streams, so normalize it here.
static inline std::string ButeMgr_NormalizePath(const char* p)
{
	std::string s(p ? p : "");
...
{
#if defined(LT_MACOS)
	std::string _bmPath = ButeMgr_NormalizePath((const char*)sAttributeFilename);
	std::ifstream* pIStream = new std::ifstream(_bmPath.c_str(), std::ios_base::in);
#elif _MSC_VER >= 1300
	std::ifstream* pIStream = new std::ifstream(sAttributeFilename, std::ios_base::in);
#else
	ifstream* pIStream = new ifstream(sAttributeFilename, ios::in);
#endif // VC7
	if( !pIStream )
		return false;
	if (pIStream->fail())
...
#if defined(LT_MACOS)
	std::string _bmPath = ButeMgr_NormalizePath((const char*)sAttributeFilename);
	std::ifstream* pIs = new std::ifstream(_bmPath.c_str(), std::ios_base::binary);
#elif _MSC_VER >= 1300
	std::ifstream* pIs = new std::ifstream(sAttributeFilename, std::ios_base::binary);
#else
	ifstream* pIs = new ifstream(sAttributeFilename, ios::binary);
#endif // VC7
	if (!pIs)
		return false;
	if (pIs->fail())
