...
#ifdef LT_MACOS
#include <dlfcn.h>   // dlopen/dlsym for the natively built libClientFx.dylib
#endif
#include "CMoveMgr.h"
#include "WinUtil.h"
...
// NOTE (LP64): szFormat is fed straight to sscanf below, so its conversions
// must match the *actual* size of the targets. The uint32 readers here use
// "%u" — NOT "%lu": `unsigned long` was 4 bytes under Win32, but is 8 on
// LP64, so "%lu" makes sscanf write 8 bytes into a 4-byte field and smash the
// next struct member (it corrupted FX_KEY/FX_GROUP data and crashed the
// ClientFX group load).
template< typename T>
...
bool CClientFXDB::Init(ILTClient* pLTClient)
{
	...
	// How many named effect GROUPS the .fxf files actually yielded. This is the
	// number that matters for "is the waterfall there": level SpecialFX objects
	// reference groups BY NAME (FxName = "JA_Waterfall"), so 17 effect *types*
	// with 0 groups would still render nothing.
	fprintf(stderr, "[fx] %d effect groups loaded from ClientFX/*.fxf\n",
	        (int)m_collGroupFX.GetSize());
...
bool CClientFXDB::LoadFxDll()
{
	...
	// ★ macOS: dlopen our natively built libClientFx.dylib.
	//
	// The whole Win32 body below was #ifdef'd out and fell through to
	// `return true` with m_nNumEffectTypes still 0, so EVERY effect lookup
	// missed and the game silently rendered no ClientFX at all -- no waterfalls,
	// mist, smoke, spray, sparks, muzzle flashes or menu background art.
	//
	// Deliberately NOT named ClientFx.fxd: the retail Win32 CLIENTFX.FXD lives in
	// the same game directory. LT_CLIENTFX_MODULE overrides the path, mirroring
	// LT_OBJECT_MODULE for the game logic (dlopen needs the leading "./").
	UnloadFxDll();

	const char *pModule = getenv("LT_CLIENTFX_MODULE");
	if (!pModule || !pModule[0])
		pModule = "./libClientFx.dylib";

	void *pLib = dlopen(pModule, RTLD_NOW | RTLD_LOCAL);
	if (!pLib)
	{
		fprintf(stderr, "[fx] dlopen(%s) FAILED: %s\n", pModule, dlerror());
		return false;
	}
	m_hDLLInst = (HINSTANCE)pLib;

	// Merge our interface database into the one the effect library built, exactly
	// as the Win32 path does -- without this the library's own holders are NULL.
	TSetMasterFn pSetMasterFn = (TSetMasterFn)dlsym(pLib, "SetMasterDatabase");
	if (pSetMasterFn)
		pSetMasterFn(GetMasterDatabase());

	// Resolve the entry points. Any missing one means the dylib is stale or the
	// export list (macbuild/exports_clientfx.txt) is out of date -- say which.
	#define FX_RESOLVE(var, type, name)                                          \
		type var = (type)dlsym(pLib, name);                                      \
		if (!var)                                                                \
		{                                                                        \
			fprintf(stderr, "[fx] %s MISSING from %s\n", name, pModule);          \
			return false;                                                        \
		}

	FX_RESOLVE(pfnNum,               FX_GETNUM,            "fxGetNum")
	FX_RESOLVE(pfnRef,               FX_GETREF,            "fxGetRef")
	FX_RESOLVE(pfnSetPlayer,         FX_SETPLAYERFUNC,     "fxSetPlayer")
	FX_RESOLVE(pfnSetAppFocus,       FX_SETAPPFOCUS,       "fxSetAppFocus")
	FX_RESOLVE(pfnDelete,            FX_DELETEFUNC,        "fxDelete")
	FX_RESOLVE(pfnSetCreateFunction, FX_SETCREATEFUNCTION, "fxSetCreateFunction")
	FX_RESOLVE(pfnCreatePropList,    FX_CREATEPROPLIST,    "fxCreatePropList")
	FX_RESOLVE(pfnFreePropList,      FX_FREEPROPLIST,      "fxFreePropList")
	#undef FX_RESOLVE
	...
	fprintf(stderr, "[fx] loaded %s: %d effect types\n", pModule, (int)m_nNumEffectTypes);
	return true;
...
void CClientFXDB::UnloadFxDll()
{
	...
	// ⚠️ Deliberately NOT dlclose(): running this library's static destructors
	// while the engine is still live is the same hazard that made the clean-quit
	// path crash (see PHASE2_HANDOFF §31a). Just drop our references; the image
	// stays mapped for the life of the process, which is what Win32 effectively
	// does too (it never FreeLibrary's the effect DLL during play).
	m_hDLLInst				= NULL;
	m_pfnSetPlayer			= NULL;
	m_pfnSetAppFocus		= NULL;
	m_pfnDeleteFX			= NULL;
