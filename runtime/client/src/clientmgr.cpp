...
#include "servermgr.h"
#ifdef LT_MACOS
#include "sys/macos/macos_music.h"   // LTMusic_Update (interactive music tick)
#endif
#include "systimer.h"
#include "iltclient.h"
...
#ifdef LT_MACOS
	// Interactive music. ILTDirectMusicMgr has no Update() of its own, so the
	// segment scheduler is ticked here — it needs a per-frame heartbeat to start
	// each wave at its musical time, retire finished streams and advance the
	// intensity chain when a segment ends.
	// ⚠️ Deliberately AFTER the early-out above: with sound invalid or disabled
	// there is nothing to schedule onto, so music stays silent with it.
	LTMusic_Update();
#endif
...
LTRESULT CClientMgr::LoadModelData(FileIdentifier *pIdent, Model* &pModel)
{
    ...
    // ★ Each of these is a SILENT model loss in a retail build (DEBUG_MODEL_REZ
    // compiles out) -- the caller then substitutes models\\default.ltb, a 1-unit
    // cube. Name the actual reason; "which of the three" is the whole diagnosis.
    if (dResult != LT_OK )
    {
        fprintf(stderr, "[model] '%s': Model::Load failed, LTRESULT %d\n", pFilename, (int)dResult);
        delete pModel;
		pModel = NULL ;
    ...
    {
        fprintf(stderr, "[model] '%s': CHILD MODEL TREES INVALID\n", pFilename);
        delete pModel;
    ...
    {
        fprintf(stderr, "[model] '%s': MISSING ONE OR MORE CHILD MODELS\n", pFilename);
        delete pModel;
