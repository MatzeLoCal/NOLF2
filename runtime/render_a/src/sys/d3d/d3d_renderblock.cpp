...
bool CD3D_RenderBlock::Load(ILTStream *pStream)
{
	...
	*pStream >> (uint32)m_nVertexCount;
	if (m_nVertexCount)
	{
		...
		// The retail NOLF2 world files store each vertex as 44 bytes:
		//   position(12) + UV0(8) + UV1(8) + color(4) + normal(12).
		// This engine source's SRBVertex additionally carries tangent + binormal
		// (24 bytes), so a single bulk read of sizeof(SRBVertex) would over-read
		// 24 bytes per vertex and desync the world stream.  Read the on-disk
		// fields explicitly to match the shipped file format.
		for (uint32 nVertLoop = 0; nVertLoop < m_nVertexCount; ++nVertLoop)
		{
			SRBVertex &cVert = m_aVertices[nVertLoop];
			...
			*pStream >> cVert.m_vNormal;
			// Tangent/binormal are not present in the retail vertex format.
			cVert.m_vTangent.Init(0.0f, 0.0f, 0.0f);
			cVert.m_vBinormal.Init(0.0f, 0.0f, 0.0f);
		}
	}
